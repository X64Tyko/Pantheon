...
// Regression test: getOrCreate() used to keep the *old* session (and its
// old userId()) on reconnect regardless of what user_id was passed in — a
// device re-paired to a different account would stay scoped under the
// previous owner (wrong account in listForUser(), stale queued commands/
// state) until the old session happened to expire on its own.
TEST(DeviceSessionManagerTest, ReconnectUnderDifferentUserGetsAFreshSession)
{
	DeviceSessionManager mgr;
	auto original = mgr.getOrCreate("dev-1", "user-a");
	original->pushCommand({{"type", "play"}}); // stale state that must not carry over

	auto reconnected = mgr.getOrCreate("dev-1", "user-b");

	EXPECT_NE(reconnected, original);
	EXPECT_EQ(reconnected->userId(), "user-b");
	EXPECT_EQ(mgr.get("dev-1"), reconnected);
	EXPECT_TRUE(mgr.listForUser("user-a").empty());
	ASSERT_EQ(mgr.listForUser("user-b").size(), 1u);
	EXPECT_EQ(mgr.listForUser("user-b")[0], reconnected);
}
