	...
	while ((pos = xml.find("<programme", pos)) != std::string::npos)
	{
		...
		if (opening.find("channel=\"kairos-1\"") != std::string::npos)
		{
			...
			if (s != std::string::npos)
			{
				...
				if (e != std::string::npos) starts.push_back(opening.substr(s, e - s));
			}
		}
		pos = tag_end + 1;
	}
...
TEST_F(EPGOutputTest, M3U_StructureAndFileOutput)
{
	...
	{
		size_t n = 0, pos = 0;
		while ((pos = m3u.find(needle, pos)) != std::string::npos)
		{
			++n;
			pos += needle.size();
		}
...
{
	// commit() calls applyToDB() which must persist at least one cursor row
	// for the show block on ch1.
