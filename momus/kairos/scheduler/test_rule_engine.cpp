...
protected:
					...
					const std::optional<int>& season_filter = std::nullopt)
	{
		...
		s.bind(2, type);
		s.bind(3, id);
		if (season_filter.has_value()) s.bind(4, *season_filter);
		else s.bind(4);
		s.exec();
	}
...
// ---------------------------------------------------------------------------
// Episode-scope Align Start — when no filler fits the remaining gap, the
// scheduler must still snap to the boundary instead of silently abandoning
// alignment (see RuleEngine.cpp's episode-alignment padding block).
// ---------------------------------------------------------------------------

TEST_F(RuleEngineTest, Project_EpisodeAlignment_SnapsToBoundaryWhenNoFillerFitsTheGap)
{
	// 25-minute episode leaves a 5-minute gap to the next 30-minute boundary.
	// The only available filler is a full hour long, so nothing is eligible
	// (see pickFillerSim's "sized" duration filter) — this exercises the
	// fallback path, not the normal filler-insertion one.
	db.get().exec("INSERT INTO episode (episode_id,show_id,season,episode,title,file_path,duration_ms)"
		" VALUES ('e_short','s1',1,10,'Short','/e_short.mkv',1500000)");
	db.get().exec("INSERT INTO episode (episode_id,show_id,season,episode,title,file_path,duration_ms)"
		" VALUES ('e_hourlong','s1',1,11,'Hour Long','/e_hourlong.mkv',3600000)");
	db.get().exec("INSERT INTO filler_list (filler_list_id, title, advancement)"
		" VALUES ('fl1','Too Long For The Gap','sequential')");
	db.get().exec("INSERT INTO filler_list_item (filler_list_id, item_type, item_id, position)"
		" VALUES ('fl1','episode','e_hourlong',0)");
	db.get().exec("INSERT INTO channel_filler_entry (channel_id, content_type, content_id, position)"
		" VALUES ('c1','filler_list','fl1',0)");

	insertBlock("b1", "episode", "00:00");
	addContent("b1", "episode", "e_short");
	db.get().exec("UPDATE block SET start_scope='episode', align_to_mins=30 WHERE block_id='b1'");

	Xoshiro256 rng(0);
	auto items = engine.project("c1", kMonNoon, 1, rng);

	ASSERT_GE(items.size(), 2u) << "expected at least two plays of the direct-episode content";
	EXPECT_EQ(items[0].wall_clock_start_ms, static_cast<int64_t>(kMonNoon) * 1000);
	EXPECT_EQ(items[1].item_type, "episode")
		<< "no filler item should appear — nothing available fit the 5-minute remainder";
	int64_t expected_next_boundary_ms = (static_cast<int64_t>(kMonNoon) + 30 * 60) * 1000;
	EXPECT_EQ(items[1].wall_clock_start_ms, expected_next_boundary_ms)
		<< "second episode must snap to the next 30-minute boundary, not start immediately "
...
TEST_F(RuleEngineTest, MaxConsecutiveEpisodes_NeverExceedsLimit)
{
	...
	for (size_t i = 1; i < items.size(); ++i)
	{
		if (items[i].show_id == items[i - 1].show_id) ++consecutive;
		else consecutive = 1;
...
TEST_F(RuleEngineTest, MaxConsecutiveEpisodes_ZeroMeansUnlimited)
{
	...
	auto items = engine.project("c1", kMonNoon, 3, rng);
	ASSERT_GE(items.size(), 2u);
	for (const auto& item : items) EXPECT_EQ(item.show_id, "s1") << "Only s1 is in the block; all items must be from s1";
}
