...
static void registerRoutes(httplib::Server& s)
{
	...
	s.Get("/Users/uid/Items", [](const httplib::Request& req, httplib::Response& res)
	{
		...
		// Empty library — a real, non-error response shape (an empty
		// "Items" array with a 200 status), not to be confused with a
		// server-error path. Regression coverage for the crash this used to
		// cause (unchecked items[0]/items[page_count-1] indexing on an empty
		// nlohmann::json array in a debug log line).
		if (type == "Series" && parent == "lib-tv-empty")
		{
			res.set_content(json{{"Items", json::array()}}.dump(), "application/json");
			return;
		}
		if (type == "Movie" && parent == "lib-movies-empty")
		{
			res.set_content(json{{"Items", json::array()}}.dump(), "application/json");
			return;
		}
...
// Regression: a library that genuinely has zero shows (as opposed to a
// server/network error) used to crash the whole process — items[0] on an
// empty nlohmann::json array is unchecked indexing, not a catchable
// json::exception, in a "Queried X to Y" debug log line. Reproduces the
// real-world "every sync-all run dies on an empty Emby library" bug.
TEST_F(JellyfinSourceTest, FetchShows_EmptyLibraryDoesNotCrash)
{
	EXPECT_TRUE(src_->fetchShows("lib-tv-empty").empty());
}
...
// Regression — see FetchShows_EmptyLibraryDoesNotCrash's own comment; the
// same unchecked items[0] bug existed in fetchMovies() too.
TEST_F(JellyfinSourceTest, FetchMovies_EmptyLibraryDoesNotCrash)
{
	EXPECT_TRUE(src_->fetchMovies("lib-movies-empty").empty());
}
...
TEST(JellyfinPushMetadata, ActorsMergedPreservingNonActorCredits)
{
	...
	for (const auto& p : people) if (p.value("Type", "") == "Director" && p.value("Name", "") == "Old Director") has_director = true;
...
TEST(JellyfinPushMetadata, DirectorReplacesExistingDirectorPreservingActor)
{
	...
	bool has_old_actor = false;
	for (const auto& p : people) if (p.value("Type", "") == "Actor" && p.value("Name", "") == "Old Actor") has_old_actor = true;
