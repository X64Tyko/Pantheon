...
static void registerRoutes(httplib::Server& s)
{
	...
	s.Get("/playlists/paged/items", [](const httplib::Request& req, httplib::Response& res)
	{
		const int start = req.has_param("X-Plex-Container-Start")
							  ? std::stoi(req.get_param_value("X-Plex-Container-Start"))
							  : 0;
		...
		{
			items.push_back({
				{"ratingKey", "pg" + std::to_string(start + i)},
	...
	// Empty library — a real, non-error response shape (a present-but-empty
	// "Metadata" array with a 200 status), not to be confused with a
	...
	s.Get("/library/sections/empty/all",
...
// json::exception, in a "Queried X to Y" debug log line.
TEST_F(PlexSourceTest, FetchShows_EmptyLibraryDoesNotCrash)
{
	EXPECT_TRUE(src_->fetchShows("empty").empty());
}
...
TEST_F(PlexSourceTest, FetchMovies_EmptyLibraryDoesNotCrash)
{
	EXPECT_TRUE(src_->fetchMovies("empty").empty());
}
