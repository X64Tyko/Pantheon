...
protected:
    ...
    ServiceContext ctx{db, conf, sync, schedule_cache, materializer, divergence_checker,
                        engine, auth, logs, dl, email};
    ...
    void insertMediaSource(const std::string& id, const std::string& base_url,
        ...
        s.bind(1, id); s.bind(2, id + " display"); s.bind(3, base_url);
        s.bind(4, auto_writeback ? 1 : 0); s.bind(5, update_art ? 1 : 0);
        s.bind(6, update_ext_ids ? 1 : 0); s.bind(7, update_collections ? 1 : 0);
        s.exec();
    }
    ...
    void mapShow(const std::string& source_id, const std::string& show_id) {
        ...
        s.bind(1, show_id); s.bind(2, source_id);
        s.exec();
    }
    ...
    void insertShow(const std::string& id, bool match_confirmed) {
        ...
        s.bind(1, id); s.bind(2, match_confirmed ? 1 : 0);
        s.exec();
    }
};
