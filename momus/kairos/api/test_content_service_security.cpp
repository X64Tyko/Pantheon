...
struct CanaryFile {
    ...
    ~CanaryFile() { std::error_code ec; fs::remove(path, ec); }
};
...
struct LegitPosterFile {
    ...
    ~LegitPosterFile() { std::error_code ec; fs::remove(path, ec); }
};
...
protected:
    ...
    void SetUp() override {
        ...
        s.bind(1, show_id); s.bind(2, "Security Test Show"); s.exec();
        SQLite::Statement m(db.get(),
            "INSERT INTO movie(movie_id, title, file_path, duration_ms) VALUES (?, ?, ?, ?)");
        m.bind(1, movie_id); m.bind(2, "Security Test Movie");
        m.bind(3, "/media/movies/Security Test Movie/movie.mkv"); m.bind(4, 5400000);
        m.exec();
    }
