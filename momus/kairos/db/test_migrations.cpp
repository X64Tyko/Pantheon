...
static std::set<std::string> tableNames(SQLite::Database& db)
{
	...
	while (q.executeStep()) out.insert(q.getColumn(0).getString());
	return out;
}
...
static std::set<std::string> columnNames(SQLite::Database& db, const std::string& table)
{
	...
	while (q.executeStep()) out.insert(q.getColumn(1).getString());
	return out;
}
...
TEST_F(MigrationTest, MigrationVersionsAreContiguousFrom1)
{
	...
	while (q.executeStep()) versions.push_back(q.getColumn(0).getInt());
	ASSERT_FALSE(versions.empty());
	for (int i = 0; i < (int)versions.size(); ++i) EXPECT_EQ(versions[i], i + 1) << "Gap or skip at position " << i;
}
...
TEST_F(MigrationTest, AllExpectedTablesExist)
{
	...
	for (const auto& t : expected) EXPECT_GT(tables.count(t), 0u) << "Missing table: " << t;
}
...
TEST_F(MigrationTest, ChannelColumnsIncludeAllMigrationAdditions)
{
	...
	EXPECT_GT(cols.count("owner_user_id"), 0u) << "added v101";
	EXPECT_GT(cols.count("is_demo"), 0u) << "added v101";
}

TEST_F(MigrationTest, UserColumnsIncludeChannelBuilderGrant)
{
	auto cols = columnNames(db.get(), "user");
	EXPECT_GT(cols.count("channel_builder_enabled"), 0u) << "added v101";
}
