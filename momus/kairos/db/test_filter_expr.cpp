...
TEST(FilterExprCompileTest, WildcardBeginsWithProducesPrefixLike)
{
	...
	EXPECT_NE(r.sql.find("LIKE ? || '%'"), std::string::npos);
	// title: matches canonical title, original_title, and item_alternate_title
	// (3 binds, all the same value) — see FilterExpr.cpp's dedicated title branch.
	ASSERT_EQ(r.binds.size(), 3u);
...
protected:
					 ...
					 const std::string& director         = "", const std::string& tags = "[]")
	{
		...
		s.bind(2, title);
		s.bind(3, std::string(""));
		s.bind(4, std::string("/x/") + id + ".mkv");
		s.bind(5, duration_ms);
		s.bind(6, year);
		s.bind(7, genres);
		s.bind(8, audience_rating);
		s.bind(9, resolution_label);
		if (added_at > 0) s.bind(10, added_at);
		else s.bind(10);
		s.bind(11, std::string("local"));
		s.bind(12, director);
		s.bind(13, tags);
		s.exec();
	}
	...
	void insertAlternateTitle(const std::string& item_type, const std::string& kairos_id,
							  const std::string& title, const std::string& language = "")
	{
		SQLite::Statement s(db.get(), R"(
            INSERT INTO item_alternate_title (item_type, kairos_id, title, language) VALUES (?,?,?,?)
        )");
		s.bind(1, item_type);
		s.bind(2, kairos_id);
		s.bind(3, title);
...
// Neither movie's canonical title contains the search word at all — only a
// stored alternate/localized title does. Both the bare-word fuzzy path and
// the explicit title: field must still find it, regardless of what language
// the alternate title is tagged with (or untagged, language="").
TEST_F(FilterExprIntegrationTest, FuzzyWordMatchesAlternateTitleOnly)
{
	insertMovie("m1", "The Dog and the Cat", "[]", 2020, 5, 6000000);
	insertMovie("m2", "Something Else", "[]", 2020, 5, 6000000);
	EXPECT_EQ(searchMovies("Perro").total, 0) << "sanity: canonical-only search shouldn't already match";

	insertAlternateTitle("movie", "m1", "El Perro y El Gato", "es");

	auto r = searchMovies("El Perro y El Gato");
	ASSERT_EQ(r.total, 1);
	ASSERT_EQ(r.items.size(), 1u);
	EXPECT_EQ(r.items[0].movie_id, "m1");
}

TEST_F(FilterExprIntegrationTest, TitleFieldMatchesAlternateTitleOnly)
{
	insertShow("s1", "The Dog and the Cat", "[]", 2020);
	insertShow("s2", "Something Else", "[]", 2020);
	insertAlternateTitle("show", "s1", "El Perro y El Gato", "es");

	EXPECT_EQ(searchShows("title:\"El Perro y El Gato\"").total, 1);
	EXPECT_EQ(searchShows("title:*Perro*").total, 1);
	EXPECT_EQ(searchShows("-title:*Perro*").total, 1) << "the other show should still match the negation";
}
...
{
	for (int i = 0; i < 20; ++i) insertMovie("m" + std::to_string(i), "Movie " + std::to_string(i), "[]", 2020, 5, 6000000);
...
TEST_F(FilterExprIntegrationTest, PlaylistOrderSortsMoviesByPlaylistPosition)
{
	...
	MovieSearchParams p;
	p.filter      = "playlist:pl1";
	p.sort        = "playlist_order";
	p.playlist_id = "pl1";
	p.limit       = 50;
	auto r        = repo.searchMovies(p);
