...
TEST_F(ChapterRepositoryTest, SyncChaptersPreservesLockedRows)
{
	...
	for (const auto& c : result) if (c.chapter_type == "ad_break" && c.locked) foundLocked = true;
...
// ---------------------------------------------------------------------------
// getShowIdsNeedingDetection — used by the standalone chapter_detection
// scheduled job (see kairos/src/api/services/JobService.cpp) to pick a
// target independent of sync's own cadence.
// ---------------------------------------------------------------------------

TEST_F(ChapterRepositoryTest, GetShowIdsNeedingDetectionExcludesShowsWithDetectedIntroOrCredits)
{
	SQLite::Statement show1(db.get(), "INSERT INTO show (show_id, title) VALUES ('needs-detect','Show A')");
	show1.exec();
	SQLite::Statement show2(db.get(), "INSERT INTO show (show_id, title) VALUES ('already-detected','Show B')");
	show2.exec();

	SQLite::Statement ep2(db.get(),
						  "INSERT INTO episode (episode_id, show_id, season, episode, title, file_path, duration_ms)"
						  " VALUES ('ep-b','already-detected',1,1,'Ep','/b.mkv',1440000)");
	ep2.exec();

	repo.syncChapters("episode", "ep-b", "detected", {makeChapter("intro", 0, 30000, 0)});

	auto ids = repo.getShowIdsNeedingDetection(10);
	EXPECT_EQ(std::count(ids.begin(), ids.end(), "needs-detect"), 1);
	EXPECT_EQ(std::count(ids.begin(), ids.end(), "already-detected"), 0);
}

TEST_F(ChapterRepositoryTest, GetShowIdsNeedingDetectionIgnoresNonDetectedOrWrongTypeChapters)
{
	SQLite::Statement show(db.get(), "INSERT INTO show (show_id, title) VALUES ('s-manual','Show')");
	show.exec();
	SQLite::Statement ep(db.get(),
						 "INSERT INTO episode (episode_id, show_id, season, episode, title, file_path, duration_ms)"
						 " VALUES ('ep-manual','s-manual',1,1,'Ep','/m.mkv',1440000)");
	ep.exec();

	// A manually-created chapter, and a detected one of an irrelevant type —
	// neither should count as "already detected" for this purpose.
	repo.syncChapters("episode", "ep-manual", "manual", {makeChapter("intro", 0, 30000, 0)});
	repo.syncChapters("episode", "ep-manual", "detected", {makeChapter("recap", 0, 5000, 1)});

	auto ids = repo.getShowIdsNeedingDetection(10);
	EXPECT_EQ(std::count(ids.begin(), ids.end(), "s-manual"), 1);
}

TEST_F(ChapterRepositoryTest, GetShowIdsNeedingDetectionRespectsLimit)
{
	for (int i = 0; i < 5; ++i)
	{
		SQLite::Statement s(db.get(), "INSERT INTO show (show_id, title) VALUES (?, 'Show')");
		s.bind(1, "show-" + std::to_string(i));
		s.exec();
	}
	EXPECT_EQ(repo.getShowIdsNeedingDetection(2).size(), 2u);
	EXPECT_EQ(repo.getShowIdsNeedingDetection(100).size(), 5u);
}