...
std::shared_ptr<DeviceSession> DeviceSessionManager::getOrCreate(const std::string& roku_device_id, const std::string& user_id)
{
	std::lock_guard lock(mtx_);
	auto it = map_.find(roku_device_id);
	if (it != map_.end() && it->second->userId() == user_id)
	{
		it->second->touch();
		return it->second;
	}
	// Either genuinely new, or reconnecting under a different user_id (the
	// device got re-paired to another account) -- treat the latter as a
	// fresh pairing rather than silently keeping the old session's user_id
	// (which would leave listForUser()'s per-account scoping pointed at the
	// wrong account until the stale session naturally expired) or carrying
	// its queued commands/last-reported state across the ownership boundary.
...
std::vector<std::shared_ptr<DeviceSession>> DeviceSessionManager::listForUser(const std::string& user_id)
{
	...
	for (auto& [id, session] : map_) if (session->userId() == user_id) out.push_back(session);
	return out;
}
