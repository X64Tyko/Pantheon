...
export const api = {
  ...
  getChannelNow:      (id: string)                                                => request<ChannelNow>('GET', `/channels/${id}/now`),
    ...
    updateChannel: (id: string, b: Partial<Pick<Channel, 'name' | 'number' | 'timezone' | 'seed' | 'default_filler_selection' | 'offline_video_path' | 'offline_image_path' | 'offline_audio_id' | 'offline_audio_type' | 'offline_audio_title' | 'logo_path' | 'anchor_hashes' | 'audio_lang' | 'subtitle_lang' | 'stream_resolution' | 'stream_video_bitrate' | 'stream_audio_bitrate' | 'force_transcode' | 'content_tag' | 'pre_seed_weeks'>> & {
        trigger_pre_seed?: boolean
    }, opts?: {
