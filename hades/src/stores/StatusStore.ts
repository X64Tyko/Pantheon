...
export class StatusStore {
    ...
    get anyRunning() {
        return this.syncing || this.matching || this.detecting
    }
    ...
    get anyBusy() {
        return this.syncing || this.matching || this.writingBack || this.detecting
    }
    ...
    private async _poll() {
        try {
            const [sync, match, writeback, settings, chapters] = await Promise.all([
                api.getSyncStatus(),
                api.getMatchStatus(),
                api.getWritebackStatus(),
                api.getSettings(),
                api.getChapterDetectStatus(),
            ])
            runInAction(() => {
                this.syncing = sync.running
                this.detecting = chapters.running
