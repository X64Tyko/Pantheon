...
// Matches SessionManager's active/idle dual cadence — see that file's
// comment for why this is a background poll rather than a per-request fetch,
// and why idle backs off instead of polling forever at the active rate.
...
{
	// Same cap reasoning as VodSessionManager::create() — see its comment.
	if (max_sessions > 0)
	{
		std::lock_guard<std::mutex> lock(mtx);
