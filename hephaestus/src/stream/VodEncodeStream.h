...
private:
    ...
    // All require mtx_ already held by the caller.
    Head* findCoveringHead(int segment_index);
    Head* spawnHead(int segment_index, int64_t position_ms, const std::vector<int64_t>& segment_start_ms, int total_segments);
    void  evictOneIfAtCap();
