...
namespace
{
	...
	{
		json out = json::array();
		for (const auto& v : arr) if (v.is_string()) out.push_back(v.get<std::string>());
		return out.dump();
	}
	...
	{
		if (item.contains("ImageTags") && item["ImageTags"].contains("Primary")) return "/Items/" + item_id + "/Images/Primary";
		return "";
	}
	...
	{
		if (item.contains("BackdropImageTags") && !item["BackdropImageTags"].empty()) return "/Items/" + item_id + "/Images/Backdrop";
		return "";
	}
	...
	{
		for (const auto& p : people) if (p.value("Type", "") == role) return p.value("Name", "");
		return "";
	}
	...
	{
		json out = json::array();
		for (const auto& p : people) if (p.value("Type", "") == role) out.push_back(p.value("Name", ""));
		return out.dump();
	}
...
std::vector<Show> JellyfinBaseSource::fetchShows(const std::string& external_lib_id)
{
	...
	while (true)
	{
		...
		try
		{
			...
			// page_count == 0 is a real, non-error response shape (an empty
			// library, or the very last page landing exactly on a page
			// boundary) — items[0]/items[page_count-1] on an empty
			// nlohmann::json array is unchecked indexing (UB, not a catchable
			// json::exception), and was crashing the whole process on any
			// empty library instead of just logging nothing for it.
			if (page_count > 0) std::cout << "[jellyfin:" << source_id_ << "] Queried  " << items[0].value("Name", "") << " to " << items[page_count - 1].value("Name", "") << '\n';
			...
			for (const auto& item : items)
			{
				...
				if (item.contains("ProductionYear") && !item["ProductionYear"].is_null()) show.year = item["ProductionYear"].get<int>();
				if (item.contains("CommunityRating") && !item["CommunityRating"].is_null()) show.audience_rating = item["CommunityRating"].get<float>();
				if (item.contains("PremiereDate") && !item["PremiereDate"].is_null()) show.originally_available_at = isoDate(item["PremiereDate"].get<std::string>());
				if (item.contains("DateCreated") && !item["DateCreated"].is_null())
				{
					int64_t created = parseIsoToEpoch(item["DateCreated"].get<std::string>());
					if (created > 0)
					{
						show.added_at        = created;
						show.added_at_source = source_id_;
					}
				}

				if (item.contains("Genres")) show.genres = jsonStringArray(item["Genres"]);
				if (item.contains("Tags")) show.labels = jsonStringArray(item["Tags"]);
				if (item.contains("ProductionLocations")) show.countries = jsonStringArray(item["ProductionLocations"]);
				if (item.contains("People")) show.actors = personNames(item["People"], "Actor");
...
std::vector<Movie> JellyfinBaseSource::fetchMovies(const std::string& external_lib_id)
{
	...
	while (true)
	{
		...
		try
		{
			...
			// See fetchShows()'s identical guard above — page_count == 0 is a
			// real response shape (empty library), not an error.
			if (page_count > 0) std::cout << "[jellyfin:" << source_id_ << "] Queried  " << items[0].value("Name", "") << " to " << items[page_count - 1].value("Name", "") << '\n';

			for (const auto& item : items)
			{
				std::string file_path;
				if (item.contains("MediaSources") && !item["MediaSources"].empty()) file_path = item["MediaSources"][0].value("Path", "");
				...
				{
					int64_t ticks = 0;
					if (item.contains("RunTimeTicks") && !item["RunTimeTicks"].is_null()) ticks = item["RunTimeTicks"].get<int64_t>();
					if (ticks <= 0 && item.contains("MediaSources") && !item["MediaSources"].empty()) ticks = item["MediaSources"][0].value("RunTimeTicks", int64_t{0});
					movie.duration_ms = ticks / 10000;
				}

				if (item.contains("ProductionYear") && !item["ProductionYear"].is_null()) movie.year = item["ProductionYear"].get<int>();
				if (item.contains("CommunityRating") && !item["CommunityRating"].is_null()) movie.audience_rating = item["CommunityRating"].get<float>();

				if (item.contains("Studios") && !item["Studios"].empty()) movie.studio = item["Studios"][0].value("Name", "");
				...
				if (item.contains("Genres")) movie.genres = jsonStringArray(item["Genres"]);
				if (item.contains("Tags")) movie.labels = jsonStringArray(item["Tags"]);
				if (item.contains("ProductionLocations")) movie.countries = jsonStringArray(item["ProductionLocations"]);
				if (item.contains("DateCreated") && !item["DateCreated"].is_null())
				{
					int64_t created = parseIsoToEpoch(item["DateCreated"].get<std::string>());
					if (created > 0)
					{
						movie.added_at        = created;
				...
				if (item.contains("UserData"))
				{
					...
					if (ud.contains("Played") && !ud["Played"].is_null()) movie.src_watched = ud["Played"].get<bool>();
					if (ud.contains("PlayCount") && !ud["PlayCount"].is_null()) movie.src_view_count = ud["PlayCount"].get<int64_t>();
					if (ud.contains("PlaybackPositionTicks") && !ud["PlaybackPositionTicks"].is_null()) movie.src_position_ms = ud["PlaybackPositionTicks"].get<int64_t>() / 10000;
	...
	std::unordered_map<int, std::string> season_names;
	{
		...
		if (sr && sr->status == 200)
		{
			try
			{
				...
				for (const auto& s : j["Items"]) if (s.contains("IndexNumber") && !s["IndexNumber"].is_null()) season_names[s["IndexNumber"].get<int>()] = s.value("Name", "");
			}
			catch (...)
			{
			}
	...
	while (true)
	{
		...
		try
		{
			...
			for (const auto& item : items)
			{
				...
				if (item.contains("PremiereDate") && !item["PremiereDate"].is_null()) ep.air_date = isoDate(item["PremiereDate"].get<std::string>());
				...
				if (item.contains("AbsoluteEpisodeNumber") && !item["AbsoluteEpisodeNumber"].is_null()) ep.absolute_index = item["AbsoluteEpisodeNumber"].get<int>();

				auto sn_it = season_names.find(season);
				if (sn_it != season_names.end()) ep.season_name = sn_it->second;

				// Watch state — see Movie.h's src_watched/src_position_ms/src_watched_at/src_view_count.
				if (item.contains("UserData"))
				{
					const auto& ud = item["UserData"];
					if (ud.contains("Played") && !ud["Played"].is_null()) ep.src_watched = ud["Played"].get<bool>();
					if (ud.contains("PlayCount") && !ud["PlayCount"].is_null()) ep.src_view_count = ud["PlayCount"].get<int64_t>();
					if (ud.contains("PlaybackPositionTicks") && !ud["PlaybackPositionTicks"].is_null()) ep.src_position_ms = ud["PlaybackPositionTicks"].get<int64_t>() / 10000;
					if (ud.contains("LastPlayedDate") && !ud["LastPlayedDate"].is_null())
					{
						int64_t epoch = parseIsoToEpoch(ud["LastPlayedDate"].get<std::string>());
	...
	while (true)
	{
		...
		try
		{
			...
			for (const auto& item : items)
			{
				...
				if (ud.contains("Played") && !ud["Played"].is_null()) st.watched = ud["Played"].get<bool>();
				if (ud.contains("PlayCount") && !ud["PlayCount"].is_null()) st.view_count = ud["PlayCount"].get<int64_t>();
				if (ud.contains("PlaybackPositionTicks") && !ud["PlaybackPositionTicks"].is_null()) st.position_ms = ud["PlaybackPositionTicks"].get<int64_t>() / 10000;
				if (ud.contains("LastPlayedDate") && !ud["LastPlayedDate"].is_null())
				{
					int64_t epoch = parseIsoToEpoch(ud["LastPlayedDate"].get<std::string>());
	...
	else
	{
		...
		else
		{
			...
			if (!fields.genres.empty())
			{
				...
				{
					/* leave existing Genres untouched on parse failure */
				}
			}
			if (!fields.labels.empty())
			{
				...
				{
					/* leave existing Tags untouched on parse failure */
				}
			}
			if (!fields.countries.empty())
			{
				...
				{
					/* leave existing ProductionLocations untouched on parse failure */
				}
			}
			...
			auto replacePeopleOfType = [&](const std::string& type, const std::vector<std::string>& names)
			{
				...
				for (const auto& p : existing_people) if (p.value("Type", "") != type) merged_people.push_back(p);
				for (const auto& name : names) if (!name.empty()) merged_people.push_back(json{{"Name", name}, {"Type", type}});
			...
			if (!fields.actors.empty())
			{
				try
				{
					...
					for (const auto& n : json::parse(fields.actors)) if (n.is_string()) names.push_back(n.get<std::string>());
					replacePeopleOfType("Actor", names);
				}
				catch (const json::exception&)
				{
					/* leave existing People untouched on parse failure */
				}
			}
...
std::vector<BrowseListItem> JellyfinBaseSource::browseCollections(const std::string& ext_lib_id)
{
	...
	if (!ext_lib_id.empty()) path += "&ParentId=" + ext_lib_id;
	auto res = get(path);
	if (!res || res->status != 200) return {};
