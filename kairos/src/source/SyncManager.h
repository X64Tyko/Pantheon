...
public:
	...
	// Signals the active sync to stop cleanly at its next phase boundary or
	// inner-loop yield point. No-op if nothing is running. The flag is
	// cleared automatically when the next sync starts, so there's no need
	// to reset it manually before triggering a new sync.
