...
std::vector<Show> PlexSource::fetchShows(const std::string& external_lib_id)
{
	...
	while (true)
	{
		...
		try
		{
			...
			// A present-but-empty "Metadata" array is a real response shape
			// (survives the .contains() check above, which only rules out the
			// key being absent entirely) — items[0] on an empty nlohmann::json
			// array is unchecked indexing (UB, not a catchable json::exception),
			// crashing the whole process instead of just logging nothing.
			if (page_count > 0) std::cout << "[plex:" << source_id_ << "] Queried  " << items[0]["title"] << " to " << items[page_count - 1]["title"] << '\n';
			...
			for (const auto& minItem : items)
			{
				...
				if (!details->body.empty())
				{
					...
					if (item.contains("Location") && !item["Location"].empty()) show.folder_path = item["Location"][0].value("path", "");
					if (item.contains("year") && !item["year"].is_null()) show.year = item["year"].get<int>();
					if (item.contains("audienceRating") && !item["audienceRating"].is_null()) show.audience_rating = item["audienceRating"].get<float>();

					if (item.contains("Rating") && !item["Rating"].empty())
					{
						for (const auto& r : item["Rating"]) show.ratings.push_back(Rating{r["image"], r["type"], r["value"]});
					}

					// Genres: [{"tag":"Drama"}, ...]
					json genres = json::array();
					if (item.contains("Genre")) for (const auto& g : item["Genre"]) genres.push_back(g.value("tag", ""));
					show.genres = genres.dump();

					// Labels
					json labels = json::array();
					if (item.contains("Label")) for (const auto& l : item["Label"]) labels.push_back(l.value("tag", ""));
					show.labels = labels.dump();

					// Network (Plex exposes it as an array)
					if (item.contains("Network") && !item["Network"].empty()) show.network = item["Network"][0].value("tag", "");

					// Actors (Role array)
					json actors = json::array();
					if (item.contains("Role")) for (const auto& r : item["Role"]) actors.push_back(r.value("tag", ""));
					show.actors = actors.dump();

					// Countries
					json countries = json::array();
					if (item.contains("Country")) for (const auto& c : item["Country"]) countries.push_back(c.value("tag", ""));
					show.countries = countries.dump();

					// Collections
					json collections = json::array();
					if (item.contains("Collection")) for (const auto& c : item["Collection"]) collections.push_back(c.value("tag", ""));
					...
					if (item.contains("Guid"))
					{
						for (const auto& g : item["Guid"])
						{
							...
							if (titleEnd != std::string::npos) show.scraper_info.push_back(ScraperInfo{id.substr(0, titleEnd), id.substr(titleEnd + 3)});
...
std::vector<Movie> PlexSource::fetchMovies(const std::string& external_lib_id)
{
	...
	while (true)
	{
		...
		try
		{
			...
			// See the identical guard in fetchShows() above.
			if (page_count > 0) std::cout << "[plex:" << source_id_ << "] Queried  " << items[0]["title"] << " to " << items[page_count - 1]["title"] << '\n';
			...
			for (const auto& item : items)
			{
				...
				if (item.contains("year") && !item["year"].is_null()) movie.year = item["year"].get<int>();
				...
				if (item.contains("audienceRating") && !item["audienceRating"].is_null()) movie.audience_rating = item["audienceRating"].get<float>();
				movie.added_at        = item.value("addedAt", int64_t{0});
				movie.added_at_source = source_id_;

				// Director
				if (item.contains("Director") && !item["Director"].empty()) movie.director = item["Director"][0].value("tag", "");
				...
				if (item.contains("viewOffset") && !item["viewOffset"].is_null()) movie.src_position_ms = item["viewOffset"].get<int64_t>();
				if (item.contains("lastViewedAt") && !item["lastViewedAt"].is_null()) movie.src_watched_at = item["lastViewedAt"].get<int64_t>();
	...
	while (true)
	{
		...
		try
		{
			...
			for (const auto& item : items)
			{
				...
				if (ep.duration_ms <= 0 && item.contains("Media") && !item["Media"].empty()) ep.duration_ms = item["Media"][0].value("duration", int64_t{0});
				ep.overview = item.value("summary", "");
				ep.air_date = air_date;
				ep.thumb    = item.value("thumb", "");
				if (item.contains("absoluteIndex") && !item["absoluteIndex"].is_null()) ep.absolute_index = item["absoluteIndex"].get<int>();
				...
				if (item.contains("viewOffset") && !item["viewOffset"].is_null()) ep.src_position_ms = item["viewOffset"].get<int64_t>();
				if (item.contains("lastViewedAt") && !item["lastViewedAt"].is_null()) ep.src_watched_at = item["lastViewedAt"].get<int64_t>();
...
static std::vector<BrowseContentItem> parseBrowseItems(const std::string& body)
{
	...
	for (const auto& item : md["Metadata"])
	{
		try
		{
			...
			entry.item_type   = (plex_type == "movie")
								  ? "movie"
								  : (plex_type == "show")
								  ? "show"
								  : "episode";
			...
			if (plex_type == "episode")
			{
				...
				if (item.contains("parentIndex") && !item["parentIndex"].is_null()) entry.season = item["parentIndex"].get<int>();
				if (item.contains("index") && !item["index"].is_null()) entry.episode = item["index"].get<int>();
			}
	...
	const std::string& id, const std::string& plex_type)
{
	...
	for (auto& item : *raw) result.push_back({item.item_type, item.external_id});
	return result;
}
...
std::vector<SourceUserInfo> parsePlexTvUsers(const std::string& body)
{
	...
	try
	{
		...
		for (const auto& u : j["users"])
		{
			...
			if (u.contains("email") && !u["email"].is_null()) info.email = u["email"].get<std::string>();
...
std::vector<SourceUserInfo> PlexSource::listServerUsers()
{
	...
	{
		// device_list[0] on an empty array (an account with zero registered
		// devices — same class of bug as fetchShows()/fetchMovies()' own
		// items[0] fix) is unchecked indexing, not a catchable exception —
		// guard it the same way rather than crashing the whole process.
	...
	if (auto res = account_client.Get("/api/v2/home/users"); res && res->status == 200) merge(parsePlexTvUsers(res->body));
	else if (!res)
	{
	...
	if (false)
	{
		...
		if (auto res = account_client.Get("/api/users"); res && res->status == 200) merge(parsePlexTvUsers(res->body));
		else if (!res)
		{
...
std::vector<ExternalWatchState> PlexSource::fetchWatchState(const std::string& external_user_id)
{
	...
	try
	{
		...
		for (const auto& item : j["MediaContainer"].value("Metadata", json::array()))
		{
			...
			if (item.contains("viewOffset") && !item["viewOffset"].is_null()) st.position_ms = item["viewOffset"].get<int64_t>();
...
namespace
{
	...
	std::vector<std::string> extractTagArray(const json& item, const char* key)
	{
		...
		if (item.contains(key)) for (const auto& t : item[key]) out.push_back(t.value("tag", ""));
		return out;
	}
					   ...
					   const std::vector<std::string>& desired)
	{
		...
		for (const auto& c : current) if (std::find(desired.begin(), desired.end(), c) == desired.end()) to_remove.push_back(c);
		...
		for (size_t i = 0; i < desired.size(); ++i) path += "&" + plex_field + "%5B" + std::to_string(i) + "%5D.tag.tag=" + plexUrlEncode(desired[i]);
	...
	std::vector<std::string> parseJsonStringArray(const std::string& json_str)
	{
		...
		try
		{
			for (const auto& v : json::parse(json_str)) if (v.is_string()) out.push_back(v.get<std::string>());
		}
							  ...
							  const WritebackFields& fields)
{
	...
	if (fields.audience_rating) path += "&audienceRating.value=" + formatRating(*fields.audience_rating) + "&audienceRating.locked=1";
		...
		if (cur_res && cur_res->status == 200)
		{
			try
			{
				...
				if (!metadata.empty())
				{
					...
					if (!fields.genres.empty()) appendTagEdit(path, "genre", extractTagArray(item, "Genre"), parseJsonStringArray(fields.genres));
					if (!fields.director.empty()) appendTagEdit(path, "director", extractTagArray(item, "Director"), {fields.director});
					if (!fields.writer.empty()) appendTagEdit(path, "writer", extractTagArray(item, "Writer"), {fields.writer});
					if (item_type == "movie" && !fields.countries.empty()) appendTagEdit(path, "country", extractTagArray(item, "Country"), parseJsonStringArray(fields.countries));
					if (!fields.collections.empty()) appendTagEdit(path, "collection", extractTagArray(item, "Collection"), parseJsonStringArray(fields.collections));
					if (!fields.labels.empty()) appendTagEdit(path, "label", extractTagArray(item, "Label"), parseJsonStringArray(fields.labels));
				}
			}
			catch (const json::exception& e)
			{
...
std::string PlexSource::machineIdentifier()
{
	...
	if (id.empty()) std::cerr << "[plex:" << source_id_ << "] failed to fetch machineIdentifier — list push unavailable\n";
									...
									const std::vector<IMediaSource::PushListItem>& items)
{
	...
	const std::string uri  = plexUrlEncode(plexItemsUri(machine_id, items));
	const std::string base = (kind == "collection")
								 ? "/library/collections/" + list_external_id
