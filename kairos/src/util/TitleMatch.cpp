...
namespace titlematch
{
	namespace
	{
		...
		// "CD1"/"CD 1"/"CD.1", "Disc1"/"Disc.2", "Part 1"/"Part.2", "pt.1" — the
		// group captures the part number. Numeric only (no "Part One"/lettered
		// discs) to keep this from firing on ordinary title words.
		const std::regex kPartMarkerRe(
			R"(\b(?:cd|disc|disk|part|pt)\.?\s*([0-9]{1,2})\b)",
			std::regex::icase
		);
	} // namespace
	...
	std::string normalizeTitle(const std::string& s)
	{
		...
		{
			if (r.starts_with(art))
			{
				r = r.substr(std::strlen(art));
				break;
			}
		}
		std::string out;
	...
	std::pair<std::string, std::optional<int>> parseReleaseTitle(const std::string& raw)
	{
		...
		std::smatch jm;
		if (std::regex_search(name, jm, kJunkTagRe) && jm.position() > 0) junk_pos = static_cast<size_t>(jm.position());

		size_t coll_pos = std::string::npos;
		std::smatch cm;
		if (std::regex_search(name, cm, kCollectionTagRe) && cm.position() > 0) coll_pos = static_cast<size_t>(cm.position());
	...
	{
		std::smatch m;
		if (!std::regex_search(raw, m, kPartMarkerRe)) return std::nullopt;

		int part_num = std::stoi(m[1].str());
		if (part_num <= 0) return std::nullopt;

		std::string base = raw.substr(0, static_cast<size_t>(m.position(0)))
			+ raw.substr(static_cast<size_t>(m.position(0)) + static_cast<size_t>(m.length(0)));
		return std::make_pair(base, part_num);
	}

	std::vector<int> groupFileParts(const std::vector<std::string>& stems)
	{
		if (stems.size() < 2) return {};

		std::string base_norm;
		std::vector<int> parts(stems.size(), 0);
		for (size_t i = 0; i < stems.size(); ++i)
		{
			auto marker = detectFilePart(stems[i]);
			if (!marker) return {};
			const auto& [base, part_num] = *marker;
			std::string norm             = normalizeTitle(base);
			if (base_norm.empty()) base_norm = norm;
			else if (norm != base_norm) return {};
			parts[i] = part_num;
		}

		std::vector<int> sorted_parts = parts;
		std::sort(sorted_parts.begin(), sorted_parts.end());
		for (size_t i = 1; i < sorted_parts.size(); ++i) if (sorted_parts[i] == sorted_parts[i - 1]) return {}; // duplicate part number
