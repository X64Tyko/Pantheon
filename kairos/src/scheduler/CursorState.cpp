...
void CursorState::applyToDB(Database& db, const std::string& channel_id) const
{
	...
	for (const auto& [key, entry] : cursors_)
	{
		...
		if (entry.episode_id.empty()) ins.bind(7);
		else ins.bind(7, entry.episode_id);
	...
	for (const auto& [block_id, bp] : block_positions_)
	{
		...
		{
			// Same block_id-deleted-out-from-under-this-projection race as the
			// media_cursor loop above (block_state.block_id REFERENCES
			// block(block_id) ON DELETE CASCADE) — this loop was missed when
			// that one was fixed, so it still rolled back the whole sp_commit
			// savepoint (including every already-inserted scheduled_program
			// row) and 500'd GET /api/channels/:id/epg for the entire channel
			// over one stale block reference. Confirmed live via the exact
			// "FOREIGN KEY constraint failed" 500 the media_cursor fix's own
			// comment describes.
			std::cerr << "[epg] WARNING: dropping stale block_state for block=" << block_id
				<< " channel=" << channel_id << " (block no longer exists): " << e.what() << '\n';
		}
		// tryReset(), not reset() — see the media_cursor loop's own comment
		// above for why this matters after a caught exec() failure.
		bsins.tryReset();
	}
	...
	if (!slot_cursors_.empty())
	{
		...
		for (const auto& [slot_id, sc] : slot_cursors_)
		{
			...
			if (sc.episode_watermark_id.empty()) su.bind(2);
			else su.bind(2, sc.episode_watermark_id);
...
CursorState CursorState::deserializeCursors(const std::string& json_str)
{
	...
	try
	{
		...
		if (j.contains("cursors"))
		{
			...
			{
				std::vector<std::string> ahead;
				if (c.contains("ahead") && c["ahead"].is_array()) for (auto& v : c["ahead"]) ahead.push_back(v.get<std::string>());
				state.setCursorPos(
		...
		if (j.contains("slot_cursors"))
		{
			...
			{
				std::vector<std::string> ahead;
				if (sc.contains("episode_ahead") && sc["episode_ahead"].is_array()) for (auto& v : sc["episode_ahead"]) ahead.push_back(v.get<std::string>());
				state.setSlotCursor(
					sc.value("slot_id", ""),
