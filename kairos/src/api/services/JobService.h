...
// Owns scheduling for the five content-pipeline jobs (sync, metadata_refresh,
// chapter_detection, writeback_sweep, media_normalize) — each just wraps an
// existing async/single-flight-guarded trigger*() method Router already has a
// reference to. The sixth scheduled job, "backup", is registered by
...
// backup-specific. Only POST /api/jobs/:name/run-now is scoped to the five
...
private:
	...
	MediaNormalizeManager& normalize_;
