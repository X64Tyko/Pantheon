...
void SourceService::registerRoutes(httplib::Server& svr)
{
	...
	svr.Post("/api/sources", [this](const Req& req, Res& res)
	{
		...
		{
			route::logErr("POST /api/sources", e);
	...
	svr.Post("/api/sources/test", [](const Req& req, Res& res)
	{
		...
		try
		{
			...
			if (source_type == "plex")
			{
				...
				{
					route::err(res, 400, "base_url and token are required");
					return;
				}
				...
				if (r->status != 200)
				{
					route::ok(res, json{
								  {"ok", false},
								  {"error", "Unexpected response: HTTP " + std::to_string(r->status)}
							  }.dump());
					return;
				}
			...
			if (source_type == "local")
			{
				if (base_url.empty())
				{
					route::err(res, 400, "base_url (directory path) is required");
					return;
				}
				std::error_code ec;
				...
				if (ec || !exists)
				{
					route::ok(res, json{
								  {"ok", false},
								  {"error", "Directory not found or not accessible: " + base_url}
							  }.dump());
					return;
				}
				route::ok(res, json{{"ok", true}}.dump());
	...
	svr.Patch("/api/sources/:id", [this](const Req& req, Res& res)
	{
		...
		try
		{
			...
			if (!repo.getSource(id))
			{
				route::err(res, 404, "source not found");
				return;
			}
			...
			if (b.contains("auto_writeback")) repo.setAutoWriteback(id, b["auto_writeback"].get<bool>());
			if (b.contains("writeback_update_art")) repo.setWritebackUpdateArt(id, b["writeback_update_art"].get<bool>());
			if (b.contains("writeback_update_external_ids")) repo.setWritebackUpdateExternalIds(id, b["writeback_update_external_ids"].get<bool>());
			if (b.contains("writeback_update_collections")) repo.setWritebackUpdateCollections(id, b["writeback_update_collections"].get<bool>());
			route::ok(res, json{{"ok", true}}.dump());
		}
		catch (const json::exception& e)
		{
	...
	svr.Post("/api/sources/:id/users/:external_id/import", [this](const Req& req, Res& res)
	{
		...
		try
		{
			...
			{
				route::err(res, 400, "role must be admin or viewer");
				return;
			}
			...
			if (!su)
			{
				route::err(res, 404, "source user not found");
				return;
			}
			...
			if (!su->imported_user_id.empty())
			{
				route::err(res, 409, "already imported");
				return;
			}
			...
			std::string username            = base_username;
			{
				...
				for (int suffix = 2; taken(username); ++suffix) username = base_username + std::to_string(suffix);
			}
			...
			if (!su->email.empty() && smtp_configured)
			{
				...
				if (uid.empty())
				{
					route::err(res, 409, "failed to create account");
					return;
				}
				user_id = uid;
	...
	svr.Put("/api/sources/:id/users/:external_id/link", [this](const Req& req, Res& res)
	{
		...
		try
		{
			...
			if (user_id.empty())
			{
				route::err(res, 400, "user_id is required");
				return;
			}
			...
			if (!su->imported_user_id.empty())
			{
				route::err(res, 409, "already linked");
				return;
			}
			...
			if (!user_exists)
			{
				route::err(res, 404, "user not found");
				return;
			}
	...
	svr.Get("/api/sources/:id/libraries/available", [this](const Req& req, Res& res)
	{
		...
		if (!src)
		{
			route::err(res, 404, "source not found or not loaded");
			return;
		}
		if (!src->isSupported())
		{
			route::err(res, 501, src->sourceType() + " not yet supported");
			return;
		}
	...
	svr.Get("/api/sources/:id/fs", [this](const Req& req, Res& res)
	{
		...
		if (!local)
		{
			route::err(res, 400, "not a local source");
			return;
		}
		std::string path = req.has_param("path") ? req.get_param_value("path") : "";
		auto entries     = local->listSubdirectories(path);
		json result      = json::array();
		for (const auto& e : entries) result.push_back({{"external_lib_id", e.external_lib_id}, {"name", e.name}, {"type", e.type}});
		route::ok(res, result.dump());
	});
	...
	svr.Get("/api/sources/:id/libraries", [this](const Req& req, Res& res)
	{
		...
		for (const auto& lib : SourceRepository(db_).listLibraries(source_id))
			result.push_back({
				{"library_id", lib.library_id}, {"source_id", lib.source_id},
				{"external_lib_id", lib.external_lib_id},
	...
	svr.Post("/api/sources/:id/libraries", [this](const Req& req, Res& res)
	{
		...
		try
		{
			...
			{
				route::err(res, 400, "external_lib_id and display_name required");
				return;
			}
		...
		{
			route::logErr("POST /api/sources/:id/libraries", e);
			route::err(res, 409, e.what());
		}
		catch (const json::exception& e)
		{
	...
	svr.Patch("/api/sources/:id/libraries/:lid", [this](const Req& req, Res& res)
	{
		...
		try
		{
			...
			if (it == libs.end())
			{
				route::err(res, 404, "library not found");
				return;
			}
			...
			if (skip_scraping && !it->skip_scraping) ContentRepository(db_).clearPendingMatchStateForLibrary(lid);
			if (b.contains("show_on_home")) repo.setLibraryShowOnHome(lid, b["show_on_home"].get<bool>());
			route::ok(res, json{{"ok", true}}.dump());
		}
		catch (const json::exception& e)
		{
	...
	svr.Get("/api/sources/:id/libraries/:lid/scraper-priority", [this](const Req& req, Res& res)
	{
		...
		{
			route::err(res, 400, "item_type must be show or movie");
			return;
		}
	...
	// Immediate subfolder names under a local library's root — used by the
	// Downloads page to offer a real "show folder" dropdown instead of free text.
	svr.Get("/api/sources/:id/libraries/:lid/show-folders", [this](const Req& req, Res& res)
	{
		if (!currentUser() || currentUser()->role != "admin")
		{
			route::err(res, 403, "Forbidden");
		...
		auto id  = req.path_params.at("id");
		auto lid = req.path_params.at("lid");
		auto lib = SourceRepository(db_).getLibrary(lid);
		if (!lib || lib->source_id != id)
		{
			route::err(res, 404, "library not found");
			return;
		}
		...
		auto entries = local->listSubdirectories(lib->external_lib_id);
		json result  = json::array();
		for (const auto& e : entries) result.push_back({{"name", e.name}});
		route::ok(res, result.dump());
	});
	...
	svr.Get("/api/sources/:id/browse/:kind", [this](const Req& req, Res& res)
	{
		...
		else if (kind == "collections")
		{
			...
			if (library_id.empty())
			{
				route::err(res, 400, "library_id required");
				return;
			}
			auto ext_lib_id = SourceRepository(db_).getExternalLibId(library_id, source_id);
			if (ext_lib_id.empty())
			{
				route::err(res, 404, "library not found for this source");
				return;
			}
		...
		{
			route::err(res, 400, "unknown kind: " + kind);
			return;
		}

		json result = json::array();
		for (const auto& item : items) result.push_back({{"id", item.id}, {"title", item.title}, {"item_count", item.item_count}});
		route::ok(res, result.dump());
	});
	...
	svr.Get("/api/sources/:id/browse/:kind/:eid", [this](const Req& req, Res& res)
	{
		...
		if (kind == "playlists") items = src->browsePlaylistItems(eid);
		else if (kind == "collections") items = src->browseCollectionItems(eid);
		else
		{
			route::err(res, 400, "unknown kind: " + kind);
			return;
