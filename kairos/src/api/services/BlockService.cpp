...
void BlockService::registerRoutes(httplib::Server& svr)
{
	...
	{
		// Ownership check only, no rate limit — this is a read, and the
		// editor re-fetches it on every load/save, so gating it through the
		// same budget as actual mutations would burn it just from normal use.
		if (!currentUser() || !channel_auth::canEditChannel(db_, *currentUser(), req.path_params.at("id")))
		{
	...
	{
		if (!currentUser() || !channel_auth::canMutateChannel(db_, guest_mutation_limiter_, *currentUser(), req.path_params.at("id")))
		{
			route::err(res, 403, "Forbidden");
			return;
		}
		...
		try
		{
			...
			if (b.contains("inter_filler"))
				repo.updateBlockField(bid, "inter_filler",
									  b["inter_filler"].is_boolean()
										  ? (b["inter_filler"].get<bool>() ? 1 : 0)
										  : b["inter_filler"].get<int>());
			if (b.contains("snap_to_group_start"))
				repo.updateBlockField(bid, "snap_to_group_start",
									  b["snap_to_group_start"].is_boolean()
										  ? (b["snap_to_group_start"].get<bool>() ? 1 : 0)
										  : b["snap_to_group_start"].get<int>());
			if (b.contains("end_time"))
			{
				if (b["end_time"].is_null() || b["end_time"].get<std::string>().empty()) repo.clearBlockField(bid, "end_time");
				else repo.updateBlockField(bid, "end_time", b["end_time"].get<std::string>());
			}
	...
	svr.Patch("/api/channels/:id/blocks/:bid/content/:cid", [this](const Req& req, Res& res)
	{
		...
		try
		{
			...
			{
				if (b["season_filter"].is_null()) repo.clearContentField(cid, "season_filter");
				else repo.updateContentField(cid, "season_filter", b["season_filter"].get<int>());
			}
	...
	svr.Post("/api/channels/:id/blocks/:bid/filler", [this](const Req& req, Res& res)
	{
		...
		try
		{
			...
			if (b.contains("season_filter") && !b["season_filter"].is_null()) resp["season_filter"] = b["season_filter"].get<int>();
			res.status = 201;
	...
	{
		// Ownership check only, no rate limit — see the identical note on
		// GET /api/channels/:id/blocks above.
		if (!currentUser() || !channel_auth::canEditChannel(db_, *currentUser(), req.path_params.at("id")))
		{
			route::err(res, 403, "Forbidden");
			return;
	...
	svr.Post("/api/channels/:id/bumpers", [this](const Req& req, Res& res)
	{
		...
		try
		{
			...
			std::optional<int> sf;
			if (b.contains("season_filter") && !b["season_filter"].is_null()) sf = b["season_filter"].get<int>();
			BlockRepository repo(db_);
