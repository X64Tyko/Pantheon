...
void ActivityService::registerRoutes(httplib::Server& svr)
{
	...
	svr.Get("/api/activity/history", [this](const Req& req, Res& res)
	{
		...
		{
			route::err(res, 400, "invalid from/to/limit");
			return;
		}

		json out = json::array();
		for (auto& r : PlaybackHistoryRepository(db_).list(user_filter, from_ms, to_ms, limit)) out.push_back(playbackHistoryRowJson(r));
		route::ok(res, out.dump());
	});
	...
	svr.Get("/api/activity/active", [this](const Req&, Res& res)
	{
		...
		json out = json::array();
		for (auto& r : PlaybackHistoryRepository(db_).listActive(now_ms - kActiveStalenessMs)) out.push_back(playbackHistoryRowJson(r));
		route::ok(res, out.dump());
	});
}