...
void ChapterService::registerRoutes(httplib::Server& svr)
{
	...
	svr.Get(R"(/api/episodes/(.+)/chapters)", [this](const Req& req, Res& res)
	{
		auto chapters = repo_.get("episode", req.matches[1].str());
		route::ok(res, ChapterRepository::toJson(chapters).dump());
	});

	svr.Get(R"(/api/movies/(.+)/chapters)", [this](const Req& req, Res& res)
	{
		auto chapters = repo_.get("movie", req.matches[1].str());
		route::ok(res, ChapterRepository::toJson(chapters).dump());
	});

	// ── Create (manual) ──────────────────────────────────────────────────────

	svr.Post(R"(/api/episodes/(.+)/chapters)", [this](const Req& req, Res& res)
	{
		if (!currentUser() || currentUser()->role != "admin")
		{
			route::err(res, 403, "Forbidden");
		...
		const std::string media_id = req.matches[1].str();
	...
	svr.Post(R"(/api/movies/(.+)/chapters)", [this](const Req& req, Res& res)
	{
		if (!currentUser() || currentUser()->role != "admin")
		{
			route::err(res, 403, "Forbidden");
	...
	svr.Post(R"(/api/episodes/(.+)/chapters/sync)", [this](const Req& req, Res& res)
	{
		if (!currentUser() || currentUser()->role != "admin")
		{
			route::err(res, 403, "Forbidden");
		...
		const std::string kairos_id = req.matches[1].str();
		...
		if (!src)
		{
			route::err(res, 422, "source not loaded");
			return;
		}
	...
	svr.Post(R"(/api/movies/(.+)/chapters/sync)", [this](const Req& req, Res& res)
	{
		if (!currentUser() || currentUser()->role != "admin")
		{
			route::err(res, 403, "Forbidden");
	...
	svr.Post(R"(/api/shows/(.+)/chapters/sync)", [this](const Req& req, Res& res)
	{
		if (!currentUser() || currentUser()->role != "admin")
		{
			route::err(res, 403, "Forbidden");
		...
		const std::string show_id = req.matches[1].str();
		auto episodes             = ContentRepository(db_).listEpisodesForShow(show_id, "");
		if (episodes.empty())
		{
			route::err(res, 404, "show not found or has no episodes");
			return;
		}
	...
	svr.Post(R"(/api/shows/(.+)/chapters/detect)", [this](const Req& req, Res& res)
	{
		if (!currentUser() || currentUser()->role != "admin")
		{
			route::err(res, 403, "Forbidden");
	...
	svr.Post(R"(/api/movies/(.+)/chapters/detect)", [this](const Req& req, Res& res)
	{
		if (!currentUser() || currentUser()->role != "admin")
		{
			route::err(res, 403, "Forbidden");
		...
		const std::string movie_id = req.matches[1].str();
		if (!ContentRepository(db_).getMovieDetail(movie_id))
		{
			route::err(res, 404, "movie not found");
			return;
		}
