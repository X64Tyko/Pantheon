...
namespace
{
	...
	void applyShowMetadata(SQLite::Database& db, const std::string& kairos_id, const Show& s)
	{
		...
		if (s.year.has_value()) app.bind(i++, s.year.value());
		else app.bind(i++);
		if (s.audience_rating.has_value()) app.bind(i++, s.audience_rating.value());
		else app.bind(i++);
		app.bind(i++, s.thumb);
		app.bind(i++, s.art);
	...
	void applyMovieMetadata(SQLite::Database& db, const std::string& kairos_id, const Movie& m)
	{
		...
		if (m.year.has_value()) app.bind(i++, m.year.value());
		else app.bind(i++);
		if (m.audience_rating.has_value()) app.bind(i++, m.audience_rating.value());
		else app.bind(i++);
		app.bind(i++, m.thumb);
		app.bind(i++, m.art);
		app.bind(i++, kairos_id);
						...
						const std::vector<std::string>& priority_order, double epsilon = 0.01)
	{
		...
		for (const auto& c : candidates)
		{
			...
			if (std::find(priority_order.begin(), priority_order.end(), c.source) != priority_order.end()) priority_near_best = true;
		}
	...
	bool isSeason(const std::string& folder)
	{
		...
		for (char c : folder) lower += static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
	...
	std::string extractShowFolder(const fs::path& p)
	{
		...
		if (fs::exists(parent) && fs::is_directory(parent))
		{
			...
			{
				if (fs::is_directory(sibling) && isSeason(sibling.path().filename().string())) return parent.filename().string();
			}
		}
		else if (fs::exists(parent)) return extractShowFolder(parent);
...
ScraperSettings ScraperManager::getSettings() const
{
	...
	for (const auto* src : {"tmdb", "tvdb", "anidb", "tvmaze", "trakt", "anilist", "wikidata"})
	{
		ScraperConfig c;
		c.source          = src;
		...
		if (src == std::string("tvdb")) c.pin = readKey(configKey(src, "pin"), "");
												...
												const std::string& exclude_kairos_id) const
{
	...
	q.bind(4, exclude_kairos_id);
	if (q.executeStep()) return q.getColumn(0).getString();
	return "";
}

ScraperManager::DuplicateMergeResult ScraperManager::mergeIfDuplicateExternalId(const std::string& item_type,
																				...
																				const std::string& external_id)
{
	...
	if (item_type == "show")
	{
		...
		dup_title              = d_dup->title;
		dup_folder             = d_dup->folder_path;
		owner_title            = d_own->title;
		owner_folder           = d_own->folder_path;
...
void ScraperManager::buildScrapers()
{
	...
	for (const auto& c : s.configs)
	{
		...
		// TVMaze, AniList, and Wikidata's public APIs are unauthenticated —
		// unlike the others, they don't need an api_key to build.
		if (c.source == "tvmaze")
		{
			tvmaze_ = std::make_unique<TvmazeScraper>();
			continue;
		}
		if (c.source == "anilist")
		{
			anilist_ = std::make_unique<AnilistScraper>();
			continue;
		}
		if (c.source == "wikidata")
		{
			wikidata_ = std::make_unique<WikidataScraper>();
			continue;
		}
...
void ScraperManager::runRefreshAll()
{
	...
	for (const auto& id : show_ids)
	{
		try
		{
			if (refreshMetadata(id, "show"))
			{
				++refreshed;
				refresh_all_refreshed_.fetch_add(1);
			}
		}
		catch (const std::exception& e)
		{
							  ...
							  const std::string& item_type)
{
	...
	{
		std::string sql = R"(
            SELECT s.show_id, s.title, COALESCE(s.year,0), s.tmdb_id, s.tvdb_id, s.nfo_confirmed
            FROM show s
            WHERE s.match_status IN ('unscraped','uncertain','unmatched')
              AND s.skip_scraping = 0
              AND NOT EXISTS (
		...
		struct ShowRow
		{
			std::string kairos_id, title, tmdb_id, tvdb_id;
			int year;
			bool nfo_confirmed;
		};
		std::vector<ShowRow> rows;
		{
			...
			while (q.executeStep())
			{
				rows.push_back({
					...
					q.getColumn(2).getInt(), q.getColumn(5).getInt() != 0
				});
			}
		}
		for (const auto& row : rows)
		{
			...
			matchShow(settings, row.kairos_id, row.title, row.year, row.tmdb_id, row.tvdb_id, row.nfo_confirmed);
		}
	}
	...
	{
		std::string sql = R"(
            SELECT m.movie_id, m.title, COALESCE(m.year,0), m.tmdb_id, COALESCE(m.file_path,''), m.nfo_confirmed
            FROM movie m
            WHERE m.match_status IN ('unscraped','uncertain','unmatched')
              AND m.skip_scraping = 0
              AND NOT EXISTS (
		...
		struct MovieRow
		{
			std::string kairos_id, title, tmdb_id, file_path;
			int year;
			bool nfo_confirmed;
		};
		...
		for (const auto& row : rows)
		{
			...
			matchMovie(settings, row.kairos_id, row.title, row.year, row.tmdb_id, row.file_path, row.nfo_confirmed);
		}
	}
...
ScraperManager::MatchSettings ScraperManager::resolveMatchSettings(const std::string& item_type, const std::string& kairos_id) const
{
	MatchSettings out;
	...
	for (const auto& lib_id : library_ids)
	{
		...
		for (const auto& src : sourceRepo().getScraperPriority(lib_id, item_type)) if (seen_priority.insert(src).second) out.priority.push_back(src);
	}
...
void ScraperManager::matchShow(const MatchSettings& settings, const std::string& kairos_id, const std::string& title,
							   ...
							   const std::string& tvdb_id, bool nfo_confirmed)
{
	...
	if (!tmdb_id.empty() || !tvdb_id.empty())
	{
		...
		if (search_title == title)
		{
			...
			// human review-queue accept. match_confirmed stays unset here, so
			// writeback-to-source still gates on an explicit human accept —
			// UNLESS nfo_confirmed says a human already confirmed this exact
			// match before (e.g. a DB wipe + rescan), in which case restoring
			// just the match but not the confirmation would be a regression
			// for that item, not a safety improvement.
			if (nfo_confirmed)
			{
				SQLite::Statement upd(db_.get(), "UPDATE show SET match_confirmed = 1 WHERE show_id = ?");
	...
	struct Cand
	{
		std::string source, ext_id, cand_title, poster, overview;
		int cand_year;
		double score;
	};
	...
	auto collect = [&](const std::string& source, const std::string& query_title, std::vector<Show> results)
	{
		...
		for (auto& r : results)
		{
			...
			else if (source == "wikidata") ext = r.show_id; // Wikidata Q-ID stored in show_id
			...
			if (it != candidates.end())
			{
				if (sc > it->score)
					*it = {
						source, ext, r.title, r.thumb, r.overview,
	...
	for (const auto& qt : query_titles)
	{
		...
		if (wikidata_)
		{
			// No server-side year filter on Wikidata's search — no
			// retry-without-year dance needed here (see WikidataScraper).
			DLOG << "[scraper]   querying wikidata for show \"" << qt << "\"\n";
			const auto t0 = std::chrono::steady_clock::now();
			auto results  = wikidata_->searchShows(qt, year);
			DLOG << "[scraper]   wikidata done in " << elapsedMs(t0, std::chrono::steady_clock::now()) << "ms\n";
			timedSearch("wikidata", qt, std::move(results));
		}
	}
...
void ScraperManager::matchMovie(const MatchSettings& settings, const std::string& kairos_id, const std::string& title,
								int year, const std::string& tmdb_id,
								const std::string& file_path, bool nfo_confirmed)
{
	// Resolve the effective search title. When the on-disk folder name disagrees
	// with the source-provided title, the folder is more trustworthy.
	std::string search_title = title;
	...
	if (!tmdb_id.empty() && search_title == title)
	{
		...
		// same local-metadata gap (release_date etc.) otherwise. Same reasoning
		// for the nfo_confirmed restore, too.
		if (nfo_confirmed)
		{
			SQLite::Statement upd(db_.get(), "UPDATE movie SET match_confirmed = 1 WHERE movie_id = ?");
			upd.bind(1, kairos_id);
			upd.exec();
		}
	...
	auto collect = [&](const std::string& source, const std::string& query_title, std::vector<Movie> results)
	{
		...
		for (auto& r : results)
		{
			...
			else if (source == "wikidata") ext = r.movie_id; // Wikidata Q-ID stored in movie_id
			if (ext.empty()) continue;
	...
	for (const auto& qt : query_titles)
	{
		...
		if (wikidata_)
		{
			DLOG << "[scraper]   querying wikidata for movie \"" << qt << "\"\n";
			const auto t0 = std::chrono::steady_clock::now();
			auto results  = wikidata_->searchMovies(qt, year);
			DLOG << "[scraper]   wikidata done in " << elapsedMs(t0, std::chrono::steady_clock::now()) << "ms\n";
			timedSearchM("wikidata", qt, std::move(results));
		}
	}
...
void ScraperManager::persistAnidbThumbLocally(const std::string& item_type, const std::string& kairos_id)
{
	...
	{
		if (auto d = ContentRepository(db_).getMovieDetail(kairos_id)) folder = ContentRepository::parentDir(d->file_path);
	}
...
AcceptResult ScraperManager::acceptCandidate(const std::string& candidate_id)
{
	...
	upd.bind(2, item_type);
	upd.bind(3, kairos_id);
	upd.exec();
	...
	try
	{
		const std::string lang_sql = item_type == "show"
										 ? R"(
	...
	try
	{
		...
		if (source == "wikidata" && wikidata_)
		{
			if (item_type == "show")
			{
				auto show = wikidata_->fetchShow(external_id, language);
				if (show) applyShowMetadata(db_.get(), kairos_id, *show);
			}
			else if (item_type == "movie")
			{
				auto movie = wikidata_->fetchMovie(external_id, language);
				if (movie) applyMovieMetadata(db_.get(), kairos_id, *movie);
			}
		}
	}
...
bool ScraperManager::rejectCandidate(const std::string& candidate_id)
{
	...
	SQLite::Statement chk(db_.get(),
						  ...
						  "WHERE item_type=? AND kairos_id=? AND accepted IS NULL");
	chk.bind(1, item_type);
	chk.bind(2, kairos_id);
	if (chk.executeStep() && chk.getColumn(0).getInt() == 0) setMatchStatus(item_type, kairos_id, "unmatched", 0.0);
												...
												int limit, int offset) const
{
	...
	for (auto& qi : out)
	{
		...
		cq.bind(1, qi.item_type);
		cq.bind(2, qi.kairos_id);
...
bool ScraperManager::unconfirmMatch(const std::string& item_type, const std::string& kairos_id)
{
	if (item_type != "show" && item_type != "movie") return false;

	SQLite::Statement q(db_.get(),
						"SELECT match_confirmed FROM " + item_type + " WHERE " + item_type + "_id = ?");
	q.bind(1, kairos_id);
	if (!q.executeStep() || q.getColumn(0).getInt() == 0) return false;

	SQLite::Statement upd(db_.get(),
						  "UPDATE " + item_type + " SET match_confirmed = 0 WHERE " + item_type + "_id = ?");
	upd.bind(1, kairos_id);
	upd.exec();
	std::cout << "[scraper] match un-confirmed for " << item_type << " " << kairos_id << "\n";
	return true;
}

int ScraperManager::unconfirmAllMatches()
{
	int count = 0;
	for (const char* item_type : {"show", "movie"})
	{
		SQLite::Statement upd(db_.get(),
							  "UPDATE " + std::string(item_type) + " SET match_confirmed = 0 WHERE match_confirmed = 1");
		upd.exec();
		count += db_.get().getChanges();
	}
	std::cout << "[scraper] un-confirmed all matches — " << count << " item(s)\n";
	return count;
}
...
bool ScraperManager::refreshMetadata(const std::string& kairos_id, const std::string& item_type)
{
	...
	for (auto rit = ids.rbegin(); rit != ids.rend(); ++rit)
	{
		...
		try
		{
			...
			else if (id.source == "wikidata" && wikidata_)
			{
				if (item_type == "show")
				{
					auto show = wikidata_->fetchShow(id.external_id, language);
					if (show)
					{
						applyShowMetadata(db_.get(), kairos_id, *show);
						any_success = true;
				...
				{
					auto movie = wikidata_->fetchMovie(id.external_id, language);
					if (movie)
					{
						applyMovieMetadata(db_.get(), kairos_id, *movie);
						any_success = true;
...
std::vector<ScraperManager::SearchResult> ScraperManager::search(const std::string& query, const std::string& content_type) const
{
	...
	auto addShow = [&](const std::string& source, const Show& s)
	{
		...
		else if (source == "wikidata") r.external_id = s.show_id;
		r.title        = s.title;
	...
	auto addMovie = [&](const std::string& source, const Movie& m)
	{
		...
		else if (source == "wikidata") r.external_id = m.movie_id;
		r.title        = m.title;
	...
	{
		id_source = "anidb";
		id_value  = query.substr(6);
	}
	else if (query.starts_with("tvmaze:") && query.size() > 7)
	{
		id_source = "tvmaze";
		id_value  = query.substr(7);
	}
	...
	{
		id_source = "anilist";
		id_value  = query.substr(8);
	}
	else if (query.starts_with("wikidata:") && query.size() > 9)
	{
		id_source = "wikidata";
		id_value  = query.substr(9);
	}
	if (!id_value.empty())
	{
		...
		if (id_source == "wikidata" && wikidata_)
		{
			if (content_type == "show" || content_type.empty())
			{
				auto s = wikidata_->fetchShow(id_value);
				if (s) addShow("wikidata", *s);
			}
			if (content_type == "movie" || content_type.empty())
			{
				auto m = wikidata_->fetchMovie(id_value);
				if (m) addMovie("wikidata", *m);
			}
		}
		return out;
	}
	...
	if (content_type == "show" || content_type.empty())
	{
		...
		if (wikidata_) for (auto& s : capped(wikidata_->searchShows(query))) addShow("wikidata", s);
	}
	if (content_type == "movie" || content_type.empty())
	{
		...
		if (wikidata_) for (auto& m : capped(wikidata_->searchMovies(query))) addMovie("wikidata", m);
	}
	return out;
}
...
std::vector<std::pair<std::string, std::string>> ScraperManager::confirmedShowSourceIds(const std::string& show_id) const
{
...
std::vector<SpecialCandidate> ScraperManager::scanSpecialsForShow(const std::string& show_id)
{
	...
	for (const auto& [source, ext_id] : source_ids)
	{
		...
		else if (source == "wikidata" && wikidata_) eps = wikidata_->fetchEpisodes(ext_id);
		else continue;

		std::vector<Episode> specials;
		for (auto& ep : eps) if (ep.season == 0) specials.push_back(std::move(ep));
		if (!specials.empty()) per_source.push_back({source, std::move(specials)});
	}
	...
	for (const auto& src : per_source)
	{
		for (const auto& ep : src.specials)
		{
			...
			{
				merged.push_back({
					ep.title, ep.overview, ep.air_date, ep.thumb,
	...
	std::vector<MovieRow> movies;
	{
		...
		while (q.executeStep()) movies.push_back({q.getColumn(0).getString(), q.getColumn(1).getString(), q.getColumn(2).getInt()});
	}
	...
	std::set<int> reported_numbers;
	{
		...
		for (const auto& m : merged)
		{
			...
			{
				try { special_year = std::stoi(m.air_date.substr(0, 4)); }
				catch (...)
				{
				}
			}
			...
			for (const auto& [score, mv] : scored)
			{
				...
				up.bind(3, m.number);
				up.bind(4, m.title);
				up.bind(5, m.overview);
				up.bind(6, m.air_date);
				up.bind(7, m.thumb);
				up.bind(8, m.source);
				up.bind(9, m.source_episode_id);
				up.bind(10, mv->id);
				up.bind(11, score);
				up.exec();
			}
...
bool ScraperManager::acceptSpecialCandidate(const std::string& candidate_id)
{
	...
	if (episode_id.empty())
	{
		...
		ins.bind(1, episode_id);
		ins.bind(2, show_id);
		ins.bind(3, number);
		ins.bind(4, title);
		ins.bind(5, movie_duration_ms);
		ins.bind(6, overview);
		ins.bind(7, air_date);
		ins.bind(8, effective_thumb);
		ins.bind(9, movie_id);
		ins.exec();
	}
	else
	{
		...
		upd.bind(1, title);
		upd.bind(2, movie_duration_ms);
		upd.bind(3, overview);
		upd.bind(4, air_date);
		upd.bind(5, effective_thumb);
		upd.bind(6, movie_id);
