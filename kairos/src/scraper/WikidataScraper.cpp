...
{
	if (entity.contains("labels") && entity["labels"].contains(lang_key)) return entity["labels"][lang_key].value("value", "");
	return "";
}

static std::string descriptionOf(const json& entity, const std::string& lang_key)
{
	if (entity.contains("descriptions") && entity["descriptions"].contains(lang_key)) return entity["descriptions"][lang_key].value("value", "");
	return "";
}

// Every label Wikidata has for this entity becomes an alternate-title
// candidate, paired with that language's description when one exists.
// Requires the caller to have fetched the entity without a &languages=
// restriction (see fetchEntity). Search/display-language data only — the
// unrelated genre/tag/network Q-ID label resolution (resolveLabels) stays
// English-only.
static std::vector<TitleTranslation> labelTranslations(const json& entity)
{
	std::vector<TitleTranslation> out;
	if (!entity.contains("labels") || !entity["labels"].is_object()) return out;
	for (auto& [lang, val] : entity["labels"].items())
	{
		std::string title = val.value("value", "");
		if (title.empty()) continue;
		out.push_back({lang, title, descriptionOf(entity, lang)});
	}
	return out;
}
...
{
	for (const auto& t : claimEntityIds(entity, "P31")) if (allowed.count(t)) return true;
	return false;
}
...
static std::optional<WikidataDate> claimEarliestDate(const json& entity, const std::string& pid)
{
	...
	for (const auto& claim : entity["claims"][pid])
	{
		...
		int year        = 0;
		try { year = std::stoi(iso.substr(0, 4)); }
		catch (...) { continue; }
		if (year <= 0) continue;
...
std::vector<WikidataScraper::Candidate> WikidataScraper::searchAndFilter(const std::string& title, const std::set<std::string>& allowed_p31)
{
	...
	// No &languages= filter — omitting it returns every label/description
	// language Wikidata has, not just "en" (see fetchEntity's identical
	// comment for why: labelTranslations() below needs the full set).
...
std::unordered_map<std::string, std::string> WikidataScraper::resolveLabels(const std::set<std::string>& qids)
{
	...
	for (size_t start = 0; start < all.size(); start += 50)
	{
		...
		try
		{
			...
			for (auto& [qid, entity] : j["entities"].items()) out[qid] = labelOf(entity, "en");
		}
		catch (...)
		{
		}
...
{
	// No &languages= filter — Wikidata returns labels/descriptions in every
	// language it has when the param is omitted, which is what lets
	// buildShow/buildMovie honor an arbitrary preferred_language and still
	// capture every other language into item_alternate_title via
	// labelTranslations(). resolveLabels() (genre/tag/studio/network Q-ID →
	// label) is unrelated to this item's own title and stays English-only.
...
std::string WikidataScraper::wikipediaOverview(const std::string& enwiki_title, std::string* thumbnail_out)
{
	...
	{
		auto j = json::parse(res->body);
		if (thumbnail_out && j.contains("thumbnail") && j["thumbnail"].contains("source")) *thumbnail_out = j["thumbnail"].value("source", "");
...
Show WikidataScraper::buildShow(const json& entity, bool full_detail, const std::string& lang)
{
	Show s;
	// No dedicated field for a Wikidata id — stashed in show_id, same
	// convention every other non-TMDB/TVDB scraper here uses.
	const std::string want = lang.empty() ? "en" : lang;
	s.title                = labelOf(entity, want);
	if (s.title.empty()) s.title = labelOf(entity, "en");
	s.overview = descriptionOf(entity, want); // short blurb; replaced below with a real summary in detail mode
	if (s.overview.empty()) s.overview = descriptionOf(entity, "en");
	...
	{
		s.year                    = date->year;
		s.originally_available_at = date->iso;
	}
	...
	if (full_detail)
	{
		...
		s.translations = labelTranslations(entity);

		// Wikipedia's summary API is English-only here (see class comment) —
		// this always wins over whatever language-matched description was
		// just picked above for the *canonical* overview. The per-language
		// description is still preserved in s.translations either way.
...
Movie WikidataScraper::buildMovie(const json& entity, bool full_detail, const std::string& lang)
{
	Movie m;
	const std::string want = lang.empty() ? "en" : lang;
	m.title                = labelOf(entity, want);
	if (m.title.empty()) m.title = labelOf(entity, "en");
	m.overview = descriptionOf(entity, want);
	if (m.overview.empty()) m.overview = descriptionOf(entity, "en");

	auto date = claimEarliestDate(entity, "P577"); // publication date
	if (date)
	{
		m.year         = date->year;
		m.release_date = date->iso;
	}
	...
	if (full_detail)
	{
		...
		m.translations = labelTranslations(entity);

		// See buildShow's identical comment — English-only Wikipedia extract
		// still wins for the canonical overview; per-language descriptions
		// live on in m.translations regardless.
		std::string enwiki_title = entity.contains("sitelinks") && entity["sitelinks"].contains("enwiki")
									   ? entity["sitelinks"]["enwiki"].value("title", "")
									   : "";
		std::string thumb;
...
std::optional<Show> WikidataScraper::fetchShow(const std::string& external_id, const std::string& lang)
{
	...
	Show s    = buildShow(entity, true, lang);
...
std::optional<Movie> WikidataScraper::fetchMovie(const std::string& external_id, const std::string& lang)
{
	auto entity = fetchEntity(external_id);
	if (entity.is_null() || entity.empty()) return std::nullopt;
	Movie m    = buildMovie(entity, true, lang);
