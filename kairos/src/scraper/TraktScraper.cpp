...
struct TraktIds
{
	std::string trakt, tvdb, tmdb, imdb;
};
...
std::vector<TitleTranslation> TraktScraper::fetchTranslations(const std::string& kind, const std::string& external_id)
{
	std::vector<TitleTranslation> out;
	auto res = get("/" + kind + "/" + external_id + "/translations");
	...
	try
	{
		...
		for (const auto& t : j)
		{
			// Trakt returns one entry per {language, country} combo — the
			// same "language" can repeat with a different title/overview per
			// country (e.g. "es" for both ES and MX). All are kept; the
			// caller promotes whichever matches the resolved
			// preferred_language into the canonical fields.
			std::string lang  = safeStr(t, "language");
			std::string title = safeStr(t, "title");
			if (lang.empty() || title.empty()) continue;
			out.push_back({lang, title, safeStr(t, "overview")});
		}
	}
	catch (const std::exception& e)
	{
		std::cerr << "[trakt] fetchTranslations parse error: " << e.what() << "\n";
	}
	return out;
}
...
std::optional<Show> TraktScraper::fetchShow(const std::string& external_id, const std::string& lang)
{
	...
	// Trakt's base show payload above is English-only (unlike TMDB/TVDB,
	// which localize via their own &language= param) — /translations is the
	// only way to get a non-English canonical title/overview here.
	show->translations = fetchTranslations("shows", external_id);
	for (const auto& t : show->translations)
	{
		if (!lang.empty() && t.language == lang)
		{
			show->title = t.title;
			if (!t.overview.empty()) show->overview = t.overview;
			break;
		}
	}
	return show;
...
std::optional<Movie> TraktScraper::fetchMovie(const std::string& external_id, const std::string& lang)
{
	...
	// See fetchShow's identical comment.
	movie->translations = fetchTranslations("movies", external_id);
	for (const auto& t : movie->translations)
	{
		if (!lang.empty() && t.language == lang)
		{
			movie->title = t.title;
			if (!t.overview.empty()) movie->overview = t.overview;
			break;
		}
	}
	return movie;
