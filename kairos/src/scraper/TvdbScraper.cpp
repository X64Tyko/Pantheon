...
// Inverse of toLang3 — used to tag item_alternate_title entries with the same
// 2-letter codes TMDB/Trakt/Wikidata use. Unknown codes pass through raw
// rather than guessing.
static std::string toLang2(const std::string& lang3)
{
	if (lang3 == "eng") return "en";
	if (lang3 == "jpn") return "ja";
	if (lang3 == "kor") return "ko";
	if (lang3 == "zho") return "zh";
	if (lang3 == "fra") return "fr";
	if (lang3 == "deu") return "de";
	if (lang3 == "spa") return "es";
	if (lang3 == "ita") return "it";
	if (lang3 == "por") return "pt";
	if (lang3 == "rus") return "ru";
	if (lang3 == "ara") return "ar";
	if (lang3 == "nld") return "nl";
	if (lang3 == "pol") return "pl";
	if (lang3 == "swe") return "sv";
	if (lang3 == "tur") return "tr";
	return lang3;
}

static std::vector<std::string> stringArray(const json& j, const std::string& key)
{
	std::vector<std::string> out;
	if (j.contains(key) && j[key].is_array()) for (const auto& v : j[key]) if (v.is_string()) out.push_back(v.get<std::string>());
	return out;
}

// TVDB's extended series/movie records list which languages actually have a
// translation available via `nameTranslations`/`overviewTranslations` (arrays
// of 3-letter codes) — used to avoid firing a /translations request per ISO
// language when most have nothing for this item. Capped generously so one
// fetch can't balloon into dozens of requests.
static std::vector<std::string> availableTranslationLangs(const json& data)
{
	std::vector<std::string> out = stringArray(data, "nameTranslations");
	for (auto& l : stringArray(data, "overviewTranslations")) if (std::find(out.begin(), out.end(), l) == out.end()) out.push_back(l);
...
Show TvdbScraper::showFromJson(const json& j)
{
	...
	if (j.contains("status") && j["status"].is_object()) s.status = safeStr(j["status"], "name");
	...
	if (j.contains("genreIds"))
	{
	} // genre IDs only — skip

	if (j.contains("network") && j["network"].is_object()) s.network = safeStr(j["network"], "name");
	else if (j.contains("network")) s.network = safeStr(j, "network");
...
Movie TvdbScraper::movieFromJson(const json& j)
{
	...
	// studios
	if (j.contains("studios") && j["studios"].is_array() && !j["studios"].empty()) m.studio = safeStr(j["studios"][0], "name");
...
std::optional<Show> TvdbScraper::fetchShow(const std::string& external_id, const std::string& lang)
{
	...
	std::vector<std::string> avail_lang3;
	try
	{
		auto j = json::parse(res->body);
		if (j.value("status", "") != "success") return std::nullopt;
		show        = showFromJson(j["data"]);
		avail_lang3 = availableTranslationLangs(j["data"]);
	}
	catch (const std::exception& e)
	{
	...
	// Override name/overview with the requested language's translation, and
	// stash every other available language as a search/display-language
	// alternate (see Show::translations).
	const std::string canonical_lang3 = toLang3(lang.empty() ? language_ : lang);
	for (const auto& lang3 : avail_lang3)
	{
		auto tr = get("/v4/series/" + external_id + "/translations/" + lang3);
		if (!tr || tr->status != 200) continue;
		try
		{
			auto tj = json::parse(tr->body);
			if (tj.value("status", "") != "success" || !tj.contains("data")) continue;
			const auto& d        = tj["data"];
			std::string name     = safeStr(d, "name");
			std::string overview = safeStr(d, "overview");
			if (lang3 == canonical_lang3 && canonical_lang3 != "eng")
			{
				if (!name.empty()) show->title = name;
				if (!overview.empty()) show->overview = overview;
			}
			if (!name.empty()) show->translations.push_back({toLang2(lang3), name, overview});
		}
		catch (...)
		{
		}
...
std::vector<Episode> TvdbScraper::fetchEpisodes(const std::string& external_id, const std::string& lang)
{
	...
	while (true)
	{
		...
		try
		{
			...
			if (eps.empty()) break;
			for (const auto& ep : eps) out.push_back(episodeFromJson(ep, external_id));
...
std::optional<Movie> TvdbScraper::fetchMovie(const std::string& external_id, const std::string& lang)
{
	...
	for (const auto& lang3 : avail_lang3)
	{
		...
		try
		{
			...
			if (!name.empty()) movie->translations.push_back({toLang2(lang3), name, overview});
		}
		catch (...)
		{
		}
