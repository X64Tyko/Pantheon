	...
	try
	{
		...
		if (j.contains("errors"))
		{
			std::string msg = j["errors"].is_array() && !j["errors"].empty()
								  ? j["errors"][0].value("message", "unknown error")
								  : "unknown error";
...
// title.native and synonyms[] are alternate-title/search candidates AniList
// carries alongside romaji/english (whichever titleFromJson didn't pick is
// captured here too). None of these carry a language tag — AniList's schema
// doesn't say what script/language "native" or a given synonym is in, so all
// go in as language='' rather than guessing. AniList has no per-language
// overview/description translation at all, unlike TMDB/TVDB/Trakt/Wikidata.
static std::vector<TitleTranslation> titleTranslationsFromJson(const json& j)
{
	std::vector<TitleTranslation> out;
	if (j.contains("title") && j["title"].is_object())
	{
		std::string romaji  = safeStr(j["title"], "romaji");
		std::string english = safeStr(j["title"], "english");
		std::string native  = safeStr(j["title"], "native");
		if (!romaji.empty()) out.push_back({"", romaji, ""});
		if (!english.empty()) out.push_back({"", english, ""});
		if (!native.empty()) out.push_back({"", native, ""});
	}
	if (j.contains("synonyms") && j["synonyms"].is_array())
	{
		for (const auto& syn : j["synonyms"])
		{
			if (syn.is_string() && !syn.get<std::string>().empty()) out.push_back({"", syn.get<std::string>(), ""});
		}
	}
	return out;
}
...
Show AnilistScraper::showFromJson(const json& j)
{
	...
	if (!start.empty())
	{
		try { s.year = std::stoi(start.substr(0, 4)); }
		catch (...)
		{
		}
	}
	...
	s.translations = titleTranslationsFromJson(j);
...
Movie AnilistScraper::movieFromJson(const json& j)
{
	...
	if (!start.empty())
	{
		try { m.year = std::stoi(start.substr(0, 4)); }
		catch (...)
		{
		}
	}
	...
	m.translations = titleTranslationsFromJson(j);
...
static constexpr const char* kMediaFields =
	"id title { romaji english native } synonyms startDate { year month day } status genres "
...
std::vector<Show> AnilistScraper::searchShows(const std::string& title, int year)
{
	...
	try
	{
		for (const auto& item : (*data)["Page"]["media"]) out.push_back(showFromJson(item));
	}
	catch (const std::exception& e)
	{
...
std::optional<Show> AnilistScraper::fetchShow(const std::string& external_id, const std::string& lang)
{
	...
	json vars;
	try { vars["id"] = std::stoi(external_id); }
...
std::vector<Movie> AnilistScraper::searchMovies(const std::string& title, int year)
{
	...
	try
	{
		for (const auto& item : (*data)["Page"]["media"]) out.push_back(movieFromJson(item));
	}
	catch (const std::exception& e)
	{
