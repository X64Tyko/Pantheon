...
namespace
{
	...
	std::vector<std::pair<std::string, std::string>> xmlAll(const std::string& xml, const std::string& tag, size_t from = 0)
	{
		const std::string open  = "<" + tag;
		const std::string close = "</" + tag + ">";
	...
	// Every parsed <title> becomes an alternate-title candidate — AniDB has no
	// per-language overview, so overview stays empty for all of these (unlike
	// TMDB/TVDB/Trakt/Wikidata). showFromXml/movieFromXml already picked the
	// canonical (best-English) title from these same tags; it's included here
	// too so item_alternate_title always has every language the source knows,
	// not just the ones that lost the pick. Untagged entries (some "synonym"
	// titles carry no xml:lang) get language="".
	std::vector<TitleTranslation> titleTranslationsFromXml(const std::string& titles_sec)
	{
		std::vector<TitleTranslation> out;
		for (auto& [tag, text] : xmlAll(titles_sec, "title"))
		{
			std::string title = xmlDecode(text);
			if (title.empty()) continue;
			out.push_back({xmlAttr(tag, "xml:lang"), title, ""});
		}
		return out;
	}
} // namespace
...
void AnidbScraper::rateLimitWait()
{
	...
	if (elapsed < kGap) std::this_thread::sleep_for(kGap - elapsed);
...
bool AnidbScraper::ensureTitleDump()
{
...

	{
		...
		if (!f)
		{
			std::cerr << "[anidb] cannot write " << kTitlesGz << '\n';
			return false;
		}
...
std::vector<AnidbScraper::TitleMatch> AnidbScraper::searchTitleDump(const std::string& query) const
{
	...
	auto finishEntry = [&]()
	{
		...
		const std::string& best = !en_official.empty()
									  ? en_official
									  : !en_any.empty()
									  ? en_any
									  : main_title;
	...
	while (std::getline(f, line))
	{
		if (line.find("<anime aid=\"") != std::string::npos)
		{
			...
			if (p1 != std::string::npos && p2 != std::string::npos) cur_aid = line.substr(p1 + 5, p2 - p1 - 5);
			continue;
		}
		if (line.find("</anime>") != std::string::npos)
		{
			finishEntry();
			continue;
		}
...
Show AnidbScraper::showFromXml(const std::string& xml, const std::string& aid)
{
	...
	s.translations = titleTranslationsFromXml(titles_sec);
	...
	if (!ratings_sec.empty())
	{
		...
		if (!rating.empty())
		{
			try { s.audience_rating = std::stof(rating); }
			catch (...)
			{
			}
		}
...
Movie AnidbScraper::movieFromXml(const std::string& xml, const std::string& aid)
{
	...
	m.translations = titleTranslationsFromXml(titles_sec);
	...
	if (!ratings_sec.empty())
	{
		...
		if (!rating.empty())
		{
			try { m.audience_rating = std::stof(rating); }
			catch (...)
			{
			}
		}
												   ...
												   const std::string& show_id)
{
	...
	for (auto& [etag, econtent] : xmlAll(eps_sec, "episode"))
	{
		...
		try { ep.episode = std::stoi(epno_text); }
		catch (...) { continue; }
		...
		if (!len.empty())
		{
			try { ep.duration_ms = std::stoll(len) * 60 * 1000; }
			catch (...)
			{
			}
		}
