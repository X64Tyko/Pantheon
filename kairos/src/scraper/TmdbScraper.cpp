...
// Parses append_to_response=translations' `translations[]` array (present on
// both /tv/{id} and /movie/{id} detail responses) into alternate-title/
// overview candidates for item_alternate_title — search/display-language
// data, separate from the single translation actually applied to
// title/overview above via the request's own &language= param. TV entries
// key the localized title under "name", movies under "title".
static std::vector<TitleTranslation> translationArray(const json& translations_j, const std::string& title_key)
{
	std::vector<TitleTranslation> out;
	if (!translations_j.is_object() || !translations_j.contains("translations")) return out;
	const auto& arr_j = translations_j["translations"];
	if (!arr_j.is_array()) return out;
	for (const auto& t : arr_j)
	{
		std::string lang = safeStr(t, "iso_639_1");
		if (lang.empty() || !t.contains("data")) continue;
		const auto& data  = t["data"];
		std::string title = safeStr(data, title_key);
		if (title.empty()) continue; // TMDB returns an empty string for languages with no actual translation
		out.push_back({lang, title, safeStr(data, "overview")});
	}
	return out;
}
...
Show TmdbScraper::showFromJson(const json& j, bool is_detail)
{
	...
	if (j.contains("vote_average") && j["vote_average"].is_number()) s.audience_rating = j["vote_average"].get<float>();
	...
	if (is_detail)
	{
		...
		if (j.contains("networks") && j["networks"].is_array() && !j["networks"].empty()) s.network = safeStr(j["networks"][0], "name");
		...
		s.translations = translationArray(j.value("translations", json::object()), "name");
	}
...
Movie TmdbScraper::movieFromJson(const json& j, bool is_detail)
{
	...
	if (j.contains("vote_average") && j["vote_average"].is_number()) m.audience_rating = j["vote_average"].get<float>();
	...
	if (is_detail)
	{
		...
		if (j.contains("external_ids")) m.imdb_id = safeStr(j["external_ids"], "imdb_id");
		...
		m.translations = translationArray(j.value("translations", json::object()), "title");
	}
...
std::optional<Show> TmdbScraper::fetchShow(const std::string& external_id, const std::string& lang)
{
	std::string path = "/3/tv/" + external_id
		...
		+ "&append_to_response=external_ids,content_ratings,keywords,translations";
...
std::optional<Movie> TmdbScraper::fetchMovie(const std::string& external_id, const std::string& lang)
{
	std::string path = "/3/movie/" + external_id
		+ "?api_key=" + api_key_
		+ "&language=" + (lang.empty() ? language_ : lang)
		+ "&append_to_response=external_ids,release_dates,credits,keywords,translations";
