...
namespace
{
	...
	bool looksLikeSeasonFolder(const std::string& folderName)
	{
		...
		for (char c : folderName) lower += static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
...
ContentRepository::ContentRepository(Database& db)
	: db_(db)
{
}
	...
	const std::string& item_type, const std::string& item_id)
{
	if (item_type == "episode")
	{
		...
		q.bind(1, item_id);
		if (q.executeStep()) return {"show", q.getColumn(0).getString(), q.getColumn(1).getString()};
		return {"show", "", ""};
	}
	if (item_type == "movie")
	{
													...
													const std::string& episode_order)
{
	...
	if (episode_order == "absolute") sql += " ORDER BY COALESCE(absolute_index, season * 10000 + episode)";
	else if (episode_order == "airdate") sql += " ORDER BY air_date, episode";
	else sql                                 += " ORDER BY season, episode";
...
std::vector<std::pair<std::string, std::string>> ContentRepository::loadListItems(const std::string& content_type,
																				  const std::string& content_id)
{
	...
	while (q.executeStep()) items.emplace_back(q.getColumn(0).getString(), q.getColumn(1).getString());
	return items;
}
														   ...
														   std::optional<int> season_filter)
{
	...
	else if (content_type == "movie")
	{
		...
		q.bind(1, content_id);
		if (q.executeStep()) items.push_back({"movie", content_id, q.getColumn(0).getInt64()});
	}
...
std::unordered_map<std::string, std::pair<std::string, int>> ContentRepository::getEpisodeGroupMap(const std::string& show_id)
{
	SQLite::Statement q(db_.get(), R"(
	...
	while (q.executeStep())
		result[q.getColumn(0).getString()] = {
			q.getColumn(1).getString(),
...
std::optional<std::string> ContentRepository::findGroupPart1(const std::string& episode_id)
{
	SQLite::Statement q(db_.get(), R"(
...
namespace
{
						   ...
						   std::string& extras, std::vector<std::string>& vals)
	{
		...
		extras += " AND ("
			...
			"  )"
			")";
		vals.push_back(ctx.user_id);
		vals.push_back(entity_type);
		vals.push_back(ctx.user_id);
		vals.push_back(entity_type);
...
ShowListResult ContentRepository::searchShows(const ShowSearchParams& p)
{
	...
	const std::string order_clause =
		(p.sort == "recently_added")
			? std::string(" ORDER BY s.added_at ") + dirFor("DESC") + ", s.rowid " + dirFor("DESC")
			:
			...
			: (p.sort == "year")
			? std::string(" ORDER BY s.year ") + dirFor("DESC")
			: (p.sort == "audience_rating")
			? std::string(" ORDER BY s.audience_rating ") + dirFor("DESC")
			: (p.sort == "duration")
			? std::string(R"( ORDER BY (
			...
			(p.sort == "playlist_order" && p.playlist_id)
			? std::string(R"( ORDER BY (
	...
	const std::string home_exclude = !p.home_only
										 ? ""
										 : R"(
        AND NOT EXISTS (
	...
	const std::string hide_empty_clause = !p.hide_empty
											  ? ""
											  : R"(
...
MovieListResult ContentRepository::searchMovies(const MovieSearchParams& p)
{
	...
	const std::string morder    =
		(p.sort == "recently_added")
			? std::string(" ORDER BY m.added_at ") + mdirFor("DESC") + ", m.rowid " + mdirFor("DESC")
			:
			...
			: (p.sort == "duration")
			? std::string(" ORDER BY m.duration_ms ") + mdirFor("DESC")
			:
			// Chronological (oldest-first) release order — see searchShows's
			// identical comment.
			(p.sort == "air_date")
			? std::string(" ORDER BY NULLIF(m.release_date,'') ") + mdirFor("ASC")
			: (p.sort == "random")
			? (p.random_seed.has_value()
...
std::optional<ShowDetail> ContentRepository::getShowDetail(const std::string& show_id, const std::string& user_id)
{
...

	{
		...
		sq.bind(1, show_id);
		while (sq.executeStep()) d.seasons.push_back({sq.getColumn(0).getInt(), sq.getColumn(1).getString()});
	}
	...
	d.folder_path = !stored_folder_path.empty()
						? stored_folder_path
...
std::optional<MovieDetail> ContentRepository::getMovieDetail(const std::string& movie_id, const std::string& user_id)
{
	...
	if (d.is_multi_part)
	{
		SQLite::Statement p(db_.get(),
							"SELECT part_num, file_path, duration_ms FROM movie_part WHERE movie_id = ? ORDER BY part_num");
		p.bind(1, movie_id);
		while (p.executeStep())
		{
			MoviePart part;
			part.part_num    = p.getColumn(0).getInt();
			part.file_path   = p.getColumn(1).getString();
			part.duration_ms = p.getColumn(2).getInt64();
...
void ContentRepository::mergeMovieInto(const std::string& target_id, const std::string& dup_id)
{
	...
	mergeMovieIntoNoTxn(target_id, dup_id);
	txn.commit();
}

// Body of mergeMovieInto with no transaction of its own — SQLite doesn't
// support nesting SQLite::Transaction, so linkMovieParts (which absorbs
// several movies and writes movie_part in one atomic operation) calls this
// directly inside its own outer transaction instead of the public wrapper.
void ContentRepository::mergeMovieIntoNoTxn(const std::string& target_id, const std::string& dup_id)
{
	...
	move.bind(1, target_id);
	move.bind(2, dup_id);
	move.bind(3, target_id);
	move.exec();
	...
	delDupCand.bind(1, dup_id);
	delDupCand.bind(2, dup_id);
	delDupCand.exec();
...
// ordered parts (target becomes part 1), reusing mergeMovieIntoNoTxn's FK
// cleanup for each absorbed movie so this doesn't duplicate that
// correctness-sensitive logic. All movie_part rows written here use
// origin='manual' and the target is locked — see SyncManager::syncMovies,
// which only ever touches origin='auto' rows and skips locked movies
// entirely, so a later sync can't silently undo this admin action.
void ContentRepository::linkMovieParts(const std::string& target_id, const std::vector<std::string>& other_movie_ids)
{
	if (other_movie_ids.empty()) throw std::runtime_error("at least one other movie is required");
	for (const auto& id : other_movie_ids) if (id == target_id) throw std::runtime_error("cannot link a movie to itself");
	...
	auto readMovie = [&](const std::string& id) -> PartSource
	{
		SQLite::Statement q(db_.get(), "SELECT file_path, duration_ms FROM movie WHERE movie_id = ?");
		q.bind(1, id);
		if (!q.executeStep()) throw std::runtime_error("movie not found: " + id);
		return {q.getColumn(0).getString(), q.getColumn(1).getInt64()};
	};

	// Captured before mergeMovieIntoNoTxn deletes each absorbed row —
	// movie_part.file_path/duration_ms is all that survives of it.
	const PartSource target_src = readMovie(target_id);
	std::vector<PartSource> other_srcs;
	other_srcs.reserve(other_movie_ids.size());
	for (const auto& id : other_movie_ids) other_srcs.push_back(readMovie(id));

	for (const auto& id : other_movie_ids) mergeMovieIntoNoTxn(target_id, id);

	// Replaces the target's existing manual parts, if any (re-linking).
	// ON DELETE CASCADE already dropped each absorbed movie's own
	// movie_part rows (if it had any) when its `movie` row was deleted above.
	{
		SQLite::Statement del(db_.get(), "DELETE FROM movie_part WHERE movie_id = ? AND origin = 'manual'");
		del.bind(1, target_id);
		del.exec();
	}
	...
	int64_t total_ms = target_src.duration_ms;
	ins.bind(1, target_id);
	ins.bind(2, 1);
	ins.bind(3, target_src.file_path);
	ins.bind(4, target_src.duration_ms);
	ins.exec();
	ins.reset();

	int part_num = 2;
	for (const auto& src : other_srcs)
	{
		ins.bind(1, target_id);
		ins.bind(2, part_num++);
		ins.bind(3, src.file_path);
		ins.bind(4, src.duration_ms);
		ins.exec();
		ins.reset();
		total_ms += src.duration_ms;
	}

	SQLite::Statement upd(db_.get(),
						  "UPDATE movie SET is_multi_part = 1, duration_ms = ?, locked = 1 WHERE movie_id = ?");
	upd.bind(1, total_ms);
	upd.bind(2, target_id);
	upd.exec();
...
// row — the safety valve for a bad auto-detection or a manual link the
// admin wants to undo. Remaining parts (if 2+) are renumbered 1..N and kept
// as origin='manual' (locked stays as-is); down to 1 or 0 parts collapses
// the movie back to a plain single-file row, same shape SyncManager would
// produce if this were really the last file in the folder.
void ContentRepository::unlinkMoviePart(const std::string& movie_id, int part_num)
{
	SQLite::Transaction txn(db_.get());
	...
	std::vector<PartRow> parts;
	{
		SQLite::Statement q(db_.get(),
							"SELECT part_num, file_path, duration_ms FROM movie_part WHERE movie_id = ? ORDER BY part_num");
		q.bind(1, movie_id);
		while (q.executeStep()) parts.push_back({q.getColumn(0).getInt(), q.getColumn(1).getString(), q.getColumn(2).getInt64()});
	}
	if (parts.empty()) throw std::runtime_error("movie has no parts: " + movie_id);

	auto it = std::find_if(parts.begin(), parts.end(),
						   [&](const PartRow& p) { return p.part_num == part_num; });
	if (it == parts.end()) throw std::runtime_error("part not found: " + std::to_string(part_num));

	std::string title;
	{
		SQLite::Statement q(db_.get(), "SELECT title FROM movie WHERE movie_id = ?");
		q.bind(1, movie_id);
		if (q.executeStep()) title = q.getColumn(0).getString();
	}

	const std::string new_id = db::generateId();
	{
		SQLite::Statement ins(db_.get(), R"(
            INSERT INTO movie (movie_id, title, file_path, duration_ms, genres, labels, actors, countries, collections)
            VALUES (?,?,?,?,'[]','[]','[]','[]','[]')
        )");
		ins.bind(1, new_id);
		ins.bind(2, title.empty() ? std::string("Untitled") : title);
		ins.bind(3, it->file_path);
		ins.bind(4, it->duration_ms);
		ins.exec();
	}
	...
	{
		SQLite::Statement del(db_.get(), "DELETE FROM movie_part WHERE movie_id = ?");
		del.bind(1, movie_id);
		del.exec();
	}
	...
	if (parts.size() < 2)
	{
		const std::string sole_file    = parts.empty() ? std::string() : parts.front().file_path;
		const int64_t sole_duration_ms = parts.empty() ? 0 : parts.front().duration_ms;
		SQLite::Statement upd(db_.get(),
							  "UPDATE movie SET is_multi_part = 0, file_path = ?, duration_ms = ?, locked = 1 WHERE movie_id = ?");
		upd.bind(1, sole_file);
		upd.bind(2, sole_duration_ms);
		upd.bind(3, movie_id);
		upd.exec();
	}
	else
	{
		...
		{
			ins.bind(1, movie_id);
			ins.bind(2, new_part_num++);
			ins.bind(3, p.file_path);
			ins.bind(4, p.duration_ms);
			ins.exec();
			ins.reset();
			total_ms += p.duration_ms;
		}
		SQLite::Statement upd(db_.get(), "UPDATE movie SET duration_ms = ?, locked = 1 WHERE movie_id = ?");
		upd.bind(1, total_ms);
		upd.bind(2, movie_id);
		upd.exec();
	}
...
// Heuristic multi-part movie grouping candidates (GitHub #3) — mirrors
// LocalSource's clusterRootMovieParts, but over standalone `movie` rows
// (any source) instead of on-disk files, surfaced for admin review since
// these are files the auto-detection at sync time either predates or
// missed (e.g. the parts came from different sources).
std::vector<MovieGroupingCandidate> ContentRepository::getMovieGroupingCandidates()
{
	...
	{
		SQLite::Statement q(db_.get(),
							"SELECT movie_id, title, file_path, year FROM movie WHERE is_multi_part = 0 AND file_path <> ''");
		while (q.executeStep())
		{
			Row r;
			r.movie_id  = q.getColumn(0).getString();
			...
			if (!q.isColumnNull(3)) r.year = q.getColumn(3).getInt();
			rows.push_back(std::move(r));
		}
	}
	...
	{
		auto marker = titlematch::detectFilePart(fs::path(rows[i].file_path).stem().string());
		if (!marker) continue;
		byBase[titlematch::normalizeTitle(marker->first)].push_back(i);
	}

	std::vector<MovieGroupingCandidate> out;
	for (auto& [base, idxs] : byBase)
	{
		...
		for (size_t i : idxs) stems.push_back(fs::path(rows[i].file_path).stem().string());
		...
		// Same confidence shape as the episode-group heuristic
		// (buildGroupingCandidates in KairosService.cpp): starts-at-1 and
		// consecutive part numbers are each worth a chunk, 3+ parts a
		// smaller bonus, capped at 100.
		int confidence = 40;
		...
		MovieGroupingCandidate cand;
		cand.base_title = rows[idxs[order.front()]].title;
		cand.confidence = std::min(confidence, 100);
		for (size_t k : order)
		{
			const auto& r = rows[idxs[k]];
			cand.parts.push_back({r.movie_id, r.title, r.file_path, r.year, part_nums[k]});
		}
...
void ContentRepository::mergeShowInto(const std::string& target_id, const std::string& dup_id)
{
	...
	std::vector<DupEp> dup_episodes;
	{
		...
		q.bind(1, dup_id);
		while (q.executeStep()) dup_episodes.push_back({q.getColumn(0).getString(), q.getColumn(1).getInt(), q.getColumn(2).getInt()});
	}
	...
	for (const auto& ep : dup_episodes)
	{
		...
		if (it != target_ep_by_se.end())
		{
			...
			moveEpMapping.bind(1, target_ep_id);
			moveEpMapping.bind(2, ep.episode_id);
			moveEpMapping.bind(3, target_ep_id);
			moveEpMapping.exec();
			moveEpMapping.reset();
			dropEpMappingLeftover.bind(1, ep.episode_id);
			dropEpMappingLeftover.exec();
			dropEpMappingLeftover.reset();
			nullCursorEp.bind(1, ep.episode_id);
			nullCursorEp.exec();
			nullCursorEp.reset();
			deleteEp.bind(1, ep.episode_id);
			deleteEp.exec();
			deleteEp.reset();
		}
		else
		{
			reparentEp.bind(1, target_id);
			reparentEp.bind(2, ep.episode_id);
			reparentEp.exec();
			reparentEp.reset();
		}
	}
	...
	moveShowMapping.bind(1, target_id);
	moveShowMapping.bind(2, dup_id);
	moveShowMapping.bind(3, target_id);
...
EpisodeListResult ContentRepository::searchEpisodes(const EpisodeSearchParams& p)
{
	...
	if (p.playlist_id)
		where += " AND EXISTS (SELECT 1 FROM playlist_item pi "
	...
	const std::string order_clause =
		(p.sort == "playlist_order" && p.playlist_id)
			...
			(p.sort == "episode_number")
			? std::string(" ORDER BY e.season ") + dirFor("ASC") + ", e.episode " + dirFor("ASC")
