...
ChannelRepository::ChannelRepository(Database& db)
	: db_(db)
{
}
...
namespace
{
	constexpr const char* kChannelColumns =
		...
		"owner_user_id, is_demo, force_transcode, pre_seed_weeks";
} // namespace

std::vector<Channel> ChannelRepository::listChannels(const std::optional<std::string>& viewer_user_id,
													 bool as_admin)
{
	std::string sql = std::string("SELECT ") + kChannelColumns + " FROM channel";
	if (!as_admin)
	{
		sql += viewer_user_id.has_value()
				   ? " WHERE is_demo = 0 OR owner_user_id = ?"
				   : " WHERE is_demo = 0";
	}
	sql += " ORDER BY number";
	SQLite::Statement q(db_.get(), sql);
	if (!as_admin && viewer_user_id.has_value()) q.bind(1, *viewer_user_id);
...
std::optional<Channel> ChannelRepository::findById(const std::string& channel_id)
{
	SQLite::Statement q(db_.get(),
						std::string("SELECT ") + kChannelColumns + " FROM channel WHERE channel_id = ?");
	q.bind(1, channel_id);
...
int ChannelRepository::countOwnedBy(const std::string& user_id)
{
	SQLite::Statement q(db_.get(), "SELECT COUNT(*) FROM channel WHERE owner_user_id = ?");
	q.bind(1, user_id);
	...
	std::optional<int> season_filter)
{
	...
	std::string title;
	{
		const char* tsql =
			content_type == "playlist"
				? "SELECT title FROM playlist    WHERE playlist_id=?"
				: content_type == "show"
				? "SELECT title FROM show        WHERE show_id=?"
				: content_type == "movie"
	...
	s.bind(3, content_id);
	s.bind(4, advancement);
	s.bind(5, weight);
	s.bind(6, position);
	if (season_filter.has_value()) s.bind(7, season_filter.value());
	else s.bind(7);
	s.exec();
