...
public:
	...
	// Shows with no detected intro/credits chapter yet — same eligibility
	// check SyncManager::syncChaptersFromFiles uses to trickle detection
	// across the library one show per sync pass, just scoped to every show
	// in the library instead of only the ones a specific source's sync just
	// touched. Used by the standalone chapter_detection job (see
	// kairos/src/api/services/JobService.cpp) to pick a target independent
	// of sync's own cadence.
	std::vector<std::string> getShowIdsNeedingDetection(int limit);
