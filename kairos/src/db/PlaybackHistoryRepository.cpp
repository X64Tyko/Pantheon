...
PlaybackHistoryRepository::PlaybackHistoryRepository(Database& db)
	: db_(db)
{
}
		...
		if (find.executeStep())
		{
			...
			SQLite::Statement upd(db_.get(), R"SQL(
				UPDATE playback_history SET
					title = ?, device_type = ?, direct_stream = ?,
		...
		else
		{
			SQLite::Statement ins(db_.get(), R"SQL(
				INSERT INTO playback_history
					(event_id, user_id, content_type, content_id, title, device_type, direct_stream,
...
namespace
{
	...
	PlaybackHistoryRow rowFromStatement(SQLite::Statement& q)
	{
		...
		r.direct_stream       = q.getColumn(6).getInt() != 0;
	...
	SELECT event_id, user_id, content_type, content_id, title, device_type, direct_stream,
	       started_at_ms, ended_at_ms, started_position_ms, last_position_ms, duration_ms, completed
