...
namespace
{
	...
	std::vector<Token> tokenize(const std::string& src)
	{
		...
		while (i < n)
		{
			...
			if (c == '(')
			{
				toks.push_back({TokKind::LParen, "("});
				++i;
				continue;
			}
			if (c == ')')
			{
				toks.push_back({TokKind::RParen, ")"});
				++i;
				continue;
			}
	...
	{
		auto n         = std::make_shared<Node>();
		n->type        = Node::Type::Word;
		n->word_value  = std::move(value);
		n->word_negate = negate;
		return n;
	}
	...
	{
		auto n          = std::make_shared<Node>();
		n->type         = Node::Type::Group;
		n->match        = std::move(match);
		n->children     = std::move(children);
		n->group_negate = negate;
		return n;
	}
	...
	NodePtr tryParseClause(const std::string& raw)
	{
		...
		if (*field == "decade" && !value.empty() && (value.back() == 's' || value.back() == 'S')) value.pop_back();
	...
	private:
		...
		NodePtr parseOr()
		{
			...
			while (peek() && peek()->kind == TokKind::Or)
			{
				next();
				children.push_back(parseAnd());
			}
		...
		NodePtr parseTerm()
		{
			...
			bool negate     = false;
			if (!raw.empty() && raw.front() == '-' && raw.size() > 1)
			{
				negate = true;
				raw    = raw.substr(1);
			}
	...
	std::optional<std::string> findWatchState(const NodePtr& node)
	{
		if (!node) return std::nullopt;
		if (node->type == Node::Type::Clause)
		{
			return node->field == "watch_state" ? std::optional(node->value) : std::nullopt;
		}
		if (node->type == Node::Type::Group)
		{
			for (const auto& c : node->children)
			{
				if (auto v = findWatchState(c)) return v;
			}
		}
		return std::nullopt;
	}
	...
	struct Compiler
	{
		...
		std::string compileClause(const Node& n)
		{
			...
			if (f == "director" || f == "writer")
			{
				...
				{
					binds.push_back(n.value);
					return "(" + c + " IS NULL OR " + c + " NOT LIKE '%' || ? || '%')";
				}
			}
			...
			if (f == "watch_state")
			{
				// Per-viewer, unlike every other field — see FilterExpr.h's
				// user_id comment. No user in context (smart-playlist refresh)
				// means this clause can't mean anything, so it no-ops rather
				// than matching nothing/everything unpredictably.
				if (user_id.empty()) return "1=1";
				if (entity == FilterEntity::Movie)
				{
					// watch_progress.completed is the rewatch count (see
					// searchMovies's identical comment) — watched is count>0,
					// in_progress is a row that exists but hasn't been
					// completed yet, unwatched is no row at all.
					std::string completedSub = "(SELECT COALESCE(MAX(wp_ws.completed),0) FROM watch_progress wp_ws "
						"WHERE wp_ws.user_id = ? AND wp_ws.content_type = 'movie' AND wp_ws.content_id = " + col("movie_id") + ")";
					std::string existsSub = "EXISTS (SELECT 1 FROM watch_progress wp_ws2 "
						"WHERE wp_ws2.user_id = ? AND wp_ws2.content_type = 'movie' AND wp_ws2.content_id = " + col("movie_id") + ")";
					if (n.value == "watched")
					{
						binds.push_back(user_id);
						return completedSub + " > 0";
					}
					if (n.value == "in_progress")
					{
						binds.push_back(user_id);
						binds.push_back(user_id);
						return "(" + completedSub + " = 0 AND " + existsSub + ")";
					}
					binds.push_back(user_id);
					return "NOT " + existsSub; // unwatched
				}
				// Show: mirrors the watched_episode_count/episode_count badge
				// logic already surfaced in MediaGrid.tsx/MediaDetailHero.tsx —
				// watched means every episode is, unwatched means none are,
				// in_progress is anything in between.
				std::string watchedSub = "(SELECT COUNT(DISTINCT wp_ws.content_id) FROM watch_progress wp_ws "
					"JOIN episode e_ws ON e_ws.episode_id = wp_ws.content_id WHERE wp_ws.user_id = ? AND wp_ws.content_type = 'episode' "
					"AND e_ws.show_id = " + col("show_id") + " AND wp_ws.completed > 0)";
				std::string totalSub = "(SELECT COUNT(*) FROM episode e_ws2 WHERE e_ws2.show_id = " + col("show_id") + ")";
				if (n.value == "watched")
				{
					binds.push_back(user_id);
					return "(" + totalSub + " > 0 AND " + watchedSub + " >= " + totalSub + ")";
				}
				if (n.value == "in_progress")
				{
					binds.push_back(user_id);
					return "(" + watchedSub + " > 0 AND " + watchedSub + " < " + totalSub + ")";
				}
				binds.push_back(user_id);
				return "(" + watchedSub + " = 0)"; // unwatched
			}
			...
			if (f == "added")
			{
				...
				{
					binds.push_back(n.value);
					return c + " >= strftime('%s','now') - (CAST(? AS INTEGER) * 86400)";
				}
				if (op == "before")
				{
					binds.push_back(n.value);
									  ...
									  const std::string& user_id)
{
	...
	Compiler compiler{entity, alias, user_id, {}};
...
std::optional<std::string> extractWatchState(const std::string& text)
{
	...
	return findWatchState(parseFilterAst(text));
}
...
{
	if (user_id.empty() || !state) return "1=1";
	std::string existsSub = "SELECT 1 FROM watch_progress wp_ews WHERE wp_ews.user_id = ? "
		"AND wp_ews.content_type = 'episode' AND wp_ews.content_id = " + episode_id_col;
	binds.push_back(user_id);
	if (*state == "watched") return "EXISTS (" + existsSub + " AND wp_ews.completed > 0)";
	if (*state == "in_progress") return "EXISTS (" + existsSub + " AND wp_ews.completed = 0)";
	return "NOT EXISTS (" + existsSub + ")"; // unwatched (default)
}