...
std::optional<NowProgramRow> ScheduleRepository::getNowProgram(const std::string& channel_id, int64_t at_sec)
{
...
std::optional<FillerFallbackRow> ScheduleRepository::getChannelFillerFallback(const std::string& channel_id)
{
	...
	//
	// channel_filler_entry is keyed generically by (content_type, content_id) —
	// filler_list_id is a legacy column nothing has written since the content_type/
	// content_id migration (v35/v36; see Database.cpp), so a JOIN against it (the
	// old shape of this query) silently matched zero rows for every entry added
	// through the current UI, regardless of content_type. Each UNION branch below
	// mirrors one of the content types ContentRepository::loadFillerItems() /
	// RuleEngine::pickFillerSim() already resolve generically, so this fallback
	// covers the same filler_list/playlist/show/movie entries the real scheduler does.
	SQLite::Statement q(db_.get(), R"(
        WITH cfe AS (
            SELECT content_type, content_id, season_filter
            FROM channel_filler_entry WHERE channel_id = ?
        )
        SELECT cand.fi_type, cand.fi_id, cand.file_path, cand.title, cand.duration_ms
        FROM (
            SELECT fli.item_type AS fi_type, fli.item_id AS fi_id,
                   COALESCE(e.file_path, m.file_path, '')     AS file_path,
                   COALESCE(e.title,     m.title,     '')     AS title,
                   COALESCE(e.duration_ms, m.duration_ms, 0)  AS duration_ms
            FROM cfe
            JOIN filler_list_item fli ON cfe.content_type = 'filler_list' AND fli.filler_list_id = cfe.content_id
            LEFT JOIN episode e ON fli.item_type='episode' AND fli.item_id=e.episode_id
            LEFT JOIN movie   m ON fli.item_type='movie'   AND fli.item_id=m.movie_id

            UNION ALL

            SELECT pi.item_type, pi.item_id,
                   COALESCE(e.file_path, m.file_path, ''),
                   COALESCE(e.title,     m.title,     ''),
                   COALESCE(e.duration_ms, m.duration_ms, 0)
            FROM cfe
            JOIN playlist_item pi ON cfe.content_type = 'playlist' AND pi.playlist_id = cfe.content_id
            ...
            SELECT 'episode', ep.episode_id,
                   COALESCE(ep.file_path, ''), COALESCE(ep.title, ''), COALESCE(ep.duration_ms, 0)
            FROM cfe
            JOIN episode ep ON cfe.content_type = 'show' AND ep.show_id = cfe.content_id
                AND (cfe.season_filter IS NULL OR ep.season = cfe.season_filter)

            UNION ALL

            SELECT 'movie', mv.movie_id,
                   COALESCE(mv.file_path, ''), COALESCE(mv.title, ''), COALESCE(mv.duration_ms, 0)
            FROM cfe
            JOIN movie mv ON cfe.content_type = 'movie' AND mv.movie_id = cfe.content_id
        ) cand
        LEFT JOIN (
            ...
            WHERE channel_id = ?
            GROUP BY item_id
        ) ph ON ph.item_id = cand.fi_id
        WHERE cand.file_path != ''
...
std::optional<OfflineFallbackRow> ScheduleRepository::getChannelOfflineConfig(const std::string& channel_id)
{
...
std::optional<std::string> ScheduleRepository::getAudioFilePath(const std::string& item_type, const std::string& item_id)
{
...
std::optional<NextProgramRow> ScheduleRepository::getNextProgram(const std::string& channel_id, int64_t now_sec)
{
...
std::vector<EpgProgramRow> ScheduleRepository::getEpgPrograms(const std::string& channel_id,
											 ...
											 const nlohmann::json& blocks)
{
	...
	for (const auto& blk : blocks)
	{
		...
		if (end_time.empty()) s.bind(7);
		else s.bind(7, end_time);
										   ...
										   const std::string& block_id)
{
	...
	q.bind(2, item_id);
	q.bind(3, channel_id);
	if (block_id.empty()) q.bind(4);
	else q.bind(4, block_id);
													  ...
													  std::time_t aired_at)
{
	...
	q.bind(1, item_id);
	q.bind(2, channel_id);
	if (block_id.empty()) q.bind(3);
	else q.bind(3, block_id);
