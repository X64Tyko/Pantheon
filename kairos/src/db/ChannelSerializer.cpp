...
ChannelSerializer::ChannelSerializer(Database& db)
	: db_(db)
	, blocks_(db)
{
}
...
json ChannelSerializer::exportChannel(const std::string& channel_id, bool deep)
{
	SQLite::Statement cq(db_.get(), R"(
        SELECT name, number, timezone, default_filler_selection, seed,
	...
	json channel_j = {
		...
		{"default_filler_selection", cq.getColumn(3).getString()},
		{"offline_video_path", cq.getColumn(5).getString()},
		{"offline_image_path", cq.getColumn(6).getString()},
		{"offline_audio_type", cq.getColumn(7).getString()},
		{"offline_audio_title", cq.getColumn(8).getString()},
		{"logo_path", cq.getColumn(9).getString()},
	};
	if (!cq.getColumn(4).isNull()) channel_j["seed"] = cq.getColumn(4).getInt();
	...
	while (bq.executeStep()) blocks.push_back(blocks_.exportBlock(bq.getColumn(0).getString(), deep));
...
json ChannelSerializer::previewImport(const json& body, bool deep)
{
	...
	for (const auto& blk : body.value("blocks", json::array()))
	{
		...
		for (const auto& item : blk.value("content", json::array()))
		{
			...
			for (const char* k : {"tvdb_id", "imdb_id", "tmdb_id", "year", "season_filter"}) if (item.contains(k) && !item[k].is_null()) out[k] = item[k];
...
ChannelSerializer::ImportResult ChannelSerializer::importChannel(const json& body, bool deep)
{
	...
	// Insert channel row
	{
		...
		SQLite::Statement s(db_.get(), R"(
            INSERT INTO channel (channel_id, name, number, timezone,
		...
		s.bind(4, tz);
		s.bind(5, ch.value("default_filler_selection", "round_robin"));
		s.bind(6, ch.value("offline_video_path", ""));
		s.bind(7, ch.value("offline_image_path", ""));
		s.bind(8, ch.value("logo_path", ""));
		s.exec();
	}
	if (ch.contains("seed") && !ch["seed"].is_null())
	{
		SQLite::Statement s(db_.get(), "UPDATE channel SET seed=? WHERE channel_id=?");
		s.bind(1, ch["seed"].get<int>());
		s.bind(2, channel_id);
		s.exec();
	}
	...
	for (const auto& fe : ch.value("default_filler_entries", json::array()))
	{
		...
		ins.bind(2, std::string("filler_list"));
		ins.bind(3, fid);
	...
	for (const auto& blk : body.value("blocks", json::array())) blocks_.importBlock(channel_id, blk, deep, unresolved);
