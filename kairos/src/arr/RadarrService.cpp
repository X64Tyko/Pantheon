...
RadarrService::RadarrService(const std::string& base_url, const std::string& api_key)
...
httplib::Result RadarrService::get(const std::string& path) {
    auto res = client_.Get(path);
    if (!res) std::cerr << "[radarr] GET " << path << " — " << httplib::to_string(res.error()) << '\n';
    else if (res->status != 200) std::cerr << "[radarr] GET " << path << " — HTTP " << res->status << '\n';
    return res;
}

httplib::Result RadarrService::post(const std::string& path, const std::string& body) {
    auto res = client_.Post(path, body, "application/json");
    if (!res) std::cerr << "[radarr] POST " << path << " — " << httplib::to_string(res.error()) << '\n';
    return res;
}

std::vector<ArrLookupResult> RadarrService::lookup(const std::string& term) {
    auto res = get("/api/v3/movie/lookup?term=" + pctEncode(term));
    ...
    try {
        ...
        for (const auto& item : j) {
            ...
            r.external_id   = std::to_string(item.value("tmdbId", 0));
        ...
        std::cerr << "[radarr] parse error (lookup): " << e.what() << '\n';
    }
    return results;
}

ArrServiceOptions RadarrService::getOptions() {
...
bool RadarrService::add(const json& add_data, const ArrAddOptions& opts) {
    json body = add_data;
